...
pairwiseAlignment(getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query) |> flank(20, both = T, start = T)),
                  getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query) |> flank(20, both = T, start = F)) |> reverseComplement(),
...
```{r ROI_pair_align}
pairwiseAlignment(getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query)),
                  getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query)) |> reverseComplement(),
                  type="global") |> writePairwiseAlignments()
```
</details>
...
pairwiseAlignment(getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query) |> flank(20, both = T, start = T)),
                  getSeq(genomes$KUM.M3.7f, cleanGaps(ROI1$query) |> flank(20, both = T, start = F)),
                  type="global") |> writePairwiseAlignments()
```
...
list(
  kum_right = getSeq(genomes$KUM.M3.7f, ROI1$query[1] |> flank(20, both = T, start = T))[[1]],
  kum_left  = getSeq(genomes$KUM.M3.7f, ROI1$query[2] |> flank(20, both = T, start = F))[[1]],
  oki       = getSeq(genomes$OKI2018.I69, ROI1[2] |> flank(20, both = T, start = F))[[1]]
...
alignFlank <- function(gb)
  pairwiseAlignment(getSeq(genomes$KUM.M3.7f, gb$query |> flank(20, both = T, start = T)),
                    getSeq(genomes$KUM.M3.7f, gb$query |> flank(20, both = T, start = F)),
                    type="global") |> writePairwiseAlignments()

alignFlank(gb[4])

alignFlank2 <- function(gb, qGenome=genomes$KUM.M3.7f, n = 8) {
  # Remove 1-bp padding
  gb$query <- gb$query -1
...
msAlignFlank <- function(gb, tGenome = genomes$OKI2018.I69, qGenome=genomes$KUM.M3.7f)
list(
...
pairwiseAlignment(genomes$OKI2018.I69$chr1[552335:552383], genomes$OSKA2016v1.9$Chr1[1730795:1731145], type="global") |> writePairwiseAlignments()
pairwiseAlignment(genomes$OKI2018.I69$chr1[552335:552383], genomes$OSKA2016v1.9$Chr1[1730795:1731145] |> reverseComplement(), type="global") |> writePairwiseAlignments()
# We confirm that flanking regions map well
pairwiseAlignment(genomes$OKI2018.I69$chr1[551988:552647], genomes$OSKA2016v1.9$Chr1[1730510:1731548] |> reverseComplement(), type="global") |> writePairwiseAlignments()
# Only one hit
#vmatchPattern(genomes$OSKA2016v1.9$Chr1[1730795:1731145], genomes$OSKA2016v1.9, with.indels = TRUE, max.mismatch = 10)
```
...
genomes$OKI2018.I69$chr1[554365:554546]   |> as.character()  # Only one perfect BLAST hit at the NCBI, to the Oki genome.
genomes$OSKA2016v1.9$Chr1[1727927:1728109] |> as.character()  # No relevant BLAST hit at the NCBI (evalues >= 1e-4)

vmatchPattern(genomes$OKI2018.I69$chr1[554365:554546], genomes$OKI2018.I69)

# unaligned mapped region (+ / -)
pairwiseAlignment(genomes$OKI2018.I69$chr1[554365:554546], genomes$OSKA2016v1.9$Chr1[1727927:1728109]                       , type="global") |> writePairwiseAlignments()
pairwiseAlignment(genomes$OKI2018.I69$chr1[554365:554546], genomes$OSKA2016v1.9$Chr1[1727927:1728109] |> reverseComplement(), type="global") |> writePairwiseAlignments()
...
pairwiseAlignment(genomes$OKI2018.I69$chr1[554168:554692], genomes$OSKA2016v1.9$Chr1[1727781:1728321] |> reverseComplement(), type="global") |> writePairwiseAlignments()
```
...
# As character for convenience of BLAST searching.
genomes$OKI2018.I69$chr1[561646:562124]   |> as.character()  # Only one perfect BLAST hit at the NCBI, to the Oki genome.
genomes$OSKA2016v1.9$Chr1[1722712:1723220] |> as.character()  # No relevant BLAST hit at the NCBI (evalues >= 1e-4)

# unaligned mapped region (+ / -)
pairwiseAlignment(genomes$OKI2018.I69$chr1[561646:562124], genomes$OSKA2016v1.9$Chr1[1722712:1723220]                       , type="global") |> writePairwiseAlignments()
pairwiseAlignment(genomes$OKI2018.I69$chr1[561646:562124], genomes$OSKA2016v1.9$Chr1[1722712:1723220] |> reverseComplement(), type="global") |> writePairwiseAlignments()

# unaligned mapped region with flanking colinear alignments.
gbs$Oki_Osa |> subsetByOverlaps(ROI4_3)
pairwiseAlignment(genomes$OKI2018.I69$chr1[561142:563743], genomes$OSKA2016v1.9$Chr1[1720807:1723727] |> reverseComplement(), type="global") |> writePairwiseAlignments()
```
...
roi <- x[4]
getSeq(genomes$OKI2018.I69, roi)

pairwiseAlignment(getSeq(genomes$OKI2018.I69, roi + 100),
                  getSeq(genomes$OSKA2016v1.9, roi$query + 100),
                  type = "global") |> writePairwiseAlignments()

roi <- x[10]
getSeq(genomes$OKI2018.I69, roi)       |> as.character()
getSeq(genomes$OSKA2016v1.9, roi$query) |> as.character()


seqs <- c(
  subsetByOverlaps(coa$Oki_Osa, granges(roi))                             |> getSeq(x=genomes$OKI2018.I69),
  subsetByOverlaps(coa$Oki_Osa, granges(roi))$query                       |> getSeq(x=genomes$OSKA2016v1.9),
