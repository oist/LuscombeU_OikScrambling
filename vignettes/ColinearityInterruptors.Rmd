...
# Multiple inversions in a row cause flag inflation (ROI7)
...
## Genes
ROI7.genes <- GenomicFeatures::genes(annots$Oki) |> subsetByOverlaps(ROI7[37:43] |> range())
mids <- ROI7.genes |> gr2dna_seg() |> dplyr::mutate(mid = (start + end) / 2) |> dplyr::pull(mid)
text <- names(ROI7.genes) |> as.character()
