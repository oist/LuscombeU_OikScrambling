...
compDistance <- function(x) {
  ...
  x[x %in% c("Oki_Osa", "Oki_Bar", "Oki_Kum", "Oki_Aom", "Oki_Nor")] <- "Oki – North"
  x[x %in% c("Osa_Oki", "Osa_Bar", "Osa_Kum", "Osa_Aom", "Osa_Nor",
             "Bar_Oki", "Bar_Osa", "Bar_Kum", "Bar_Aom", "Bar_Nor")] <- "North – North"
  x
}
...
ggplot(rbind(
  ...
  ) |> dplyr::mutate(Distance = compDistance(name))) + aes(Region, value) +
...
```{r gbreaks_annot}
countGbsMatchingReps <- function(gb)  sum(sapply(gb$rep, \(x) ! any(isNA(x))))
 fracGbsMatchingReps <- function(gbl) sapply(gbl, countGbsMatchingReps) / sapply(gbl, length)

summaryMatchingReps <- (sapply(list(aligned    = gbs,     unAligned = unal,
             mapped     = coa,     unMapped  = unmap,
             unAlMapped = unalMap,
             transposed = tra,
             mapped2    = coa2,    unMapped2 = unmap2),
        fracGbsMatchingReps))
sapply(summaryMatchingReps |> as.data.frame(), median)
```
