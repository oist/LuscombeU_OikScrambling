...
getSeq_ <- function(x) {
  seqs <- getSeq(x)
  metadata(seqs)$genome <- unique(genome(x))
  seqs
}
...
alnNonCoa_seqs <- sapply(gbs[1:15], \(gb) gb[gb$nonCoa] |> getSeq_()) |> SimpleList()
...
```{r search_in_unmap}
unmap_seqs <- sapply(unmap[1:15], getSeq_) |> SimpleList()
...
# inv_seqs <- sapply(coa[1:15], leftInversionGaps) |> sapply(getSeq_) |> SimpleList()
...
invSeq <-  sapply(invRegions, getSeq) |> SimpleList()
```
...
## Inverted regions are AT-rich as usual.
invRegions <- sapply(coa[1:15], \(gr) filterInversions(flagInversions(gr))) |> SimpleList()
invSeq <-  sapply(invRegions, getSeq_) |> SimpleList()
...
hist(pwmHits$OKI2018.I69$score)
hist(pwmHits$OSKA2016v1.9$score)

pwmHit <- pwmHits$OKI2018.I69
...
length(subsetByOverlaps(pwmHits$OKI2018.I69, inv.lgaps$Oki_Osa))
length(subsetByOverlaps(pwmHits$OKI2018.I69, invRegions$Oki_Osa))

hist(score(subsetByOverlaps(pwmHits$OKI2018.I69, inv.lgaps$Oki_Osa)))
hist(score(subsetByOverlaps(pwmHits$OKI2018.I69, invRegions$Oki_Osa)))

# Scores in gaps are higher than average.
t.test(score(pwmHits$OKI2018.I69), score(subsetByOverlaps(pwmHits$OKI2018.I69, inv.lgaps$Oki_Osa)))

# Scores in gaps and inversions are not different 
t.test(score(subsetByOverlaps(pwmHits$OKI2018.I69, invRegions$Oki_Osa)), score(subsetByOverlaps(pwmHits$OKI2018.I69, inv.lgaps$Oki_Osa)))
