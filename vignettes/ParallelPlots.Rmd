...
contigsToMerge        <- c("contig_27_1", "contig_90_1", "contig_3_1", "contig_88_1")
contigsToMerge_start  <- c(1, cumsum(seqlengths(x$query)[contigsToMerge]))    |> unname() |> head(-1)
contigsToMerge_end    <-     (cumsum(seqlengths(x$query)[contigsToMerge]) -1) |> unname()
...
Chromosome 2 is mostly comprised of `contig_16_1`, on the short arm, and
`contig_24_1` followed by the reverse complement of `contig_79_1`,
`contig_37_1`, `contig_81_1` on the long arm.

```{r Kum_chr2_contigs_to_merge}
contigsToMerge        <- c("contig_16_1", "contig_24_1", "contig_79_1", "contig_37_1", "contig_81_1")
contigsToMerge_start  <- c(1, cumsum(seqlengths(x$query)[contigsToMerge]))    |> unname() |> head(-1)
contigsToMerge_end    <-     (cumsum(seqlengths(x$query)[contigsToMerge]) -1) |> unname()
...
plotApairOfChrs(x |> swap(), "chr2", dna_seg_scale=TRUE, main = "Kum vs Oki (double-collapsed)", annotations = annot)
```
...
```{r plot_intersect_GBs_with_tree_with_Kume, fig.width=15, fig.height=8, dev="svg", fig.ext="svg"}
