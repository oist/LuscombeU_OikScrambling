...
load_genomic_breaks_ <- function(file, genomeT, genomeQ, longShort=NULL) {
  ...
  if (! is.null(longShort)) {
    gb <-  flagLongShort(gb, longShort[[genomeT]])
    if (genomeQ %in% names(longShort)) {
      gb$query <- flagLongShort(gb$query, longShort[[genomeQ]])
    }
  }
  gb
}
...
of the telomeric regions) and contain non-overlapping aligned, unaligned
and syntenic regions.  These are `GRanges` objects, not `GBreaks`, since they
...
wgo <- mapply(makeWholeGenomeObject, gbs, unal, coa) |> SimpleList()
```
...
unalMap <- BiocParallel::bplapply(gbs, bridgeRegions) |> as("SimpleList")

unalMap[Oik_Oik] <- BiocParallel::bplapply(unalMap[Oik_Oik], flagLongShort_, longShort)
unalMap[Oik_Oik] <- annotateWithReps(unalMap[Oik_Oik], reps)
