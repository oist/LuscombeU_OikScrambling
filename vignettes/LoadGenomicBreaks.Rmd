...
unal <- sapply(unal, flagAll) |> SimpleList()
```
...
unmap <- sapply(unmap, flagAll) |> SimpleList()
```
...
unalMap <- sapply(unalMap, flagAll) |> SimpleList()
```
...
unmap2 <- sapply(unmap2, flagAll) |> SimpleList()
```
...
tra2 <- annotateWithReps.GBreaksSimpleList(tra2, reps)
tra2 <- annotateWithGenes(tra2, annots)
