...
annotateWithGenes.GRanges <- function(x, annotT, annotQ = NULL) {
  x$genes <- CAGEr:::ranges2names(x, GenomicFeatures::genes(annotT))
  x$genes[x$genes == ''] <- NA
  x
}
...
removeTranslocationsOnTargetGenome <- function(gb) {
  ...
  gb <- gb[-which(gb$tra & (gb$tdist < 200 | gb$qdist < 200)) + 1]
  coalesce_contigs(gb)
}
...
system("echo '10e745615e150adcc92cf783da1f4de3  BreakPoints.Rdata' | md5sum -c", intern = TRUE)
```
