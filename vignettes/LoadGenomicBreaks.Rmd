...
df <- align_summary[,c("align_frac", "sim")]
df$pairname <- rownames(df)
df <- tidyr::pivot_longer(df, c("align_frac", "sim"))
df$dist <- OikScrambling:::compDistance(df$pairname)
df$genus <- "Oikopleura"
df$genus[grepl("Int", df$dist)] <- "Ciona"
df <- df[!grepl("– Oki", df$dist),]

library(ggplot2)

ggplot(df) +
  aes(value, name, pch = dist, col= genus) +
  geom_point() +
