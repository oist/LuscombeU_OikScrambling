---
title: "Load genomic break data"
author: 
 - "Charles Plessy"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{Load genomic break data}
  %\VignetteEncoding{UTF-8}
editor_options: 
  chunk_output_type: console
---

```{r setup}
knitr::opts_chunk$set(cache = TRUE)
knitr::opts_knit$set(verbose = TRUE)
```

Introduction
============

This vignette loads the genomic breaks data, processes it and saves the
resulting objects in a serialised R object (`RData` file) for reuse in other
vignettes.

Similar objects are stored in lists to avoid cluttering the environment.

Pacakges
========

Core packages that provide functions we use a lot
-------------------------------------------------

```{r loadlib, echo=T, results='hide'}
suppressPackageStartupMessages({
  library('GenomicBreaks')
  library('BSgenome')
  library('GenomicFeatures')
  library('ggplot2')
})
```

BSgenome packages
-----------------

Example command to install one _Oikopleura_ BSgenome package if you do not yet
have it:

```
install.packages("BSgenome.Odioica.local.OKI2018.I69", repos="https://oist.github.io/plessy_oikgenomes_drat/")
```

```{r load_BSgenome_packages}
# These will need to be installed by the user
library("BSgenome.Odioica.local.OKI2018.I69")
library("BSgenome.Odioica.local.OSKA2016v1.9")
library("BSgenome.Odioica.local.Bar2.p4")
library("BSgenome.Odioica.local.KUM.M3")
library("BSgenome.Odioica.local.AOM.5")
library("BSgenome.Odioica.local.Odioica.reference.v3.0")

genomes <- SimpleList(
  # Chromosome assemblies
  Oki = OKI2018_I69, Osa = OSKA2016v1.9, Bar = Bar2_p4,
  # Less contiguous assemblies
  Kum = KUM_M3, Aom = AOM_5, Nor = OdB3)
```


Data packages
-------------

In the same repository: the _BreakpointsData_ package, that installs copies
of the files found in _deigo_ under `/bucket/LuscombeU/common/Breakpoints`.

```
install.packages("BreakpointsData", repos="https://oist.github.io/plessy_oikgenomes_drat/")
```

The package itself does not contain any function but loading it will give its
version in `sessionInfo()`.

```{r load_BreakpointsData}
library("BreakpointsData")
```

Genomic breaks
==============

Long/short arm information
--------------------------

```{r long_and_short_arms}

loadLongShort <- function(file, genome) {
  file <- system.file(paste0("extdata/Annotations/", file), package = "BreakpointsData")
  gr <- rtracklayer::import.gff3(file)
  gr$source <- gr$type <- gr$score <- gr$phase <- NULL
  GRanges(gr, seqinfo = seqinfo(genome), strand = "*")
}

longShort <- SimpleList()
longShort$Oki <- loadLongShort("OKI2018_I69.v2/OKI2018_I69.arms.gff3", OKI2018_I69)
longShort$Osa <- loadLongShort("OSKA2016v1.9/OSKA2016v1.9.arms.gff3", OSKA2016v1.9)
longShort$Bar <- loadLongShort("Bar2_p4.Flye/Bar2_p4.Flye.arms.gff3", Bar2_p4)

longShort
```

Mapped alignments
-----------------

Let's load data from genome alignments, copied in the `Oidioi_pairwise_v3`
directory.

```{r GRanges object}
# Local function to add extra annotation on long/short arms
longShortAnnotate <- function(gb, longShort) {
  # Setting a minimum overlap to solve problem with gbs$Bar_Kum[2383] that matches two arms.
  # In any case, this approach is too fragile...
  overlapHits <- findOverlaps(gb, longShort, minoverlap = 10) |> subjectHits()
  gb$Arm <- longShort[overlapHits]$Type
  gb
}

# Local function to add extra annotation on long/short arms while loading data
load_genomic_breaks_ <- function(file, genomeT, genomeQ, longShort=NULL) {
  file <- system.file(paste0("extdata/Oidioi_pairwise_v3/", file), package = "BreakpointsData")
  gb <- load_genomic_breaks(file, genomeT,  genomeQ)
  if (! is.null(longShort)) {
    gb <- subsetByOverlaps(gb, longShort)
    gb <- longShortAnnotate(gb, longShort)
  }
  gb
}


gbs <- SimpleList()

gbs$Oki_Osa <- load_genomic_breaks_("OKI2018_I69_1.0__OSKA2016v1.9.gff.gz", OKI2018_I69,  OSKA2016v1.9, longShort$Oki)
gbs$Oki_Bar <- load_genomic_breaks_("OKI2018_I69_1.0__Bar2_p4.gff.gz",      OKI2018_I69,  Bar2_p4,      longShort$Oki)
gbs$Oki_Kum <- load_genomic_breaks_("OKI2018_I69_1.0__KUM-M3-7f.gff.gz",    OKI2018_I69,  KUM_M3,       longShort$Oki)
gbs$Oki_Aom <- load_genomic_breaks_("OKI2018_I69_1.0__AOM-5-5f.gff.gz",     OKI2018_I69,  AOM_5,        longShort$Oki)
gbs$Oki_Nor <- load_genomic_breaks_("OKI2018_I69_1.0__OdB3.gff.gz",         OKI2018_I69,  OdB3,         longShort$Oki)

gbs$Osa_Oki <- load_genomic_breaks_("OSKA2016v1.9__OKI2018_I69_1.0.gff.gz", OSKA2016v1.9, OKI2018_I69,  longShort$Osa)
gbs$Osa_Bar <- load_genomic_breaks_("OSKA2016v1.9__Bar2_p4.gff.gz",         OSKA2016v1.9, Bar2_p4,      longShort$Osa)
gbs$Osa_Kum <- load_genomic_breaks_("OSKA2016v1.9__KUM-M3-7f.gff.gz",       OSKA2016v1.9, KUM_M3,       longShort$Osa)
gbs$Osa_Aom <- load_genomic_breaks_("OSKA2016v1.9__AOM-5-5f.gff.gz",        OSKA2016v1.9, AOM_5,        longShort$Osa)
gbs$Osa_Nor <- load_genomic_breaks_("OSKA2016v1.9__OdB3.gff.gz",            OSKA2016v1.9, OdB3,         longShort$Osa)

gbs$Bar_Oki <- load_genomic_breaks_("Bar2_p4__OKI2018_I69_1.0.gff.gz",      Bar2_p4,     OKI2018_I69,   longShort$Bar)
gbs$Bar_Osa <- load_genomic_breaks_("Bar2_p4__OSKA2016v1.9.gff.gz",         Bar2_p4,     OSKA2016v1.9,  longShort$Bar)
gbs$Bar_Kum <- load_genomic_breaks_("Bar2_p4__KUM-M3-7f.gff.gz",            Bar2_p4,     KUM_M3,        longShort$Bar)
gbs$Bar_Aom <- load_genomic_breaks_("Bar2_p4__AOM-5-5f.gff.gz",             Bar2_p4,     AOM_5,         longShort$Bar)
gbs$Bar_Nor <- load_genomic_breaks_("Bar2_p4__OdB3.gff.gz",                 Bar2_p4,     OdB3,          longShort$Bar)
```

Coalescing alignments
---------------------

```{r coalescing algorithm}
coa <- sapply(gbs, coalesce_contigs) |> SimpleList()
coa[ 1:5]  <- sapply(coa[ 1:5],  longShortAnnotate, longShort$Oki)
coa[ 6:10] <- sapply(coa[ 6:10], longShortAnnotate, longShort$Osa)
coa[11:15] <- sapply(coa[11:15], longShortAnnotate, longShort$Bar)
```

Coalesce again
--------------

See the Genomic Breaks vignette for justification.

```{r re_coalesce}
coa2 <- sapply(coa, coalesce_contigs, minwidth = 500) |> SimpleList()
coa2[ 1:5]  <- sapply(coa2[ 1:5],  longShortAnnotate, longShort$Oki)
coa2[ 6:10] <- sapply(coa2[ 6:10], longShortAnnotate, longShort$Osa)
coa2[11:15] <- sapply(coa2[11:15], longShortAnnotate, longShort$Bar)
```

Numbers of blocks
-----------------

```{r lengths}
sapply(gbs,  length)
sapply(coa,  length)
sapply(coa2, length)
```

Summary
-------

```{r summary}
df <- rbind(
  gbs = sapply(gbs, length),
  coa = sapply(coa, length),
  coa2 = sapply(coa2, length)
) |> as.data.frame()

df$what <- row.names(df)
df <- reshape2::melt(df)

ggplot(df) +
  aes(variable, value, fill = what) +
  geom_bar( stat = "identity" ) +
  ggplot2::coord_flip()
```

Save data
=========

```{r save_data}
save(gbs, coa, coa2, file = "BreakPoints.Rdata")
system("echo '4e6633c045a3d0ad56eb2061e8e27119  BreakPoints.Rdata' | md5sum -c", intern = TRUE)
```

Session information
===================

```{r session_info}
sessionInfo()
```
