...
metaPerWindow <- function(genomeWindows, meta, includeMetaCols=NULL, meta_prefix=NULL) {
...
grWindows <- function(genome, meta, windowSize=10000, meta_prefix=NULL){
  ...
  mpw <- metaPerWindow(genome, meta, meta_prefix=meta_prefix)
  mpw
}
...
ggplot(as.data.frame(chrw$Oki |> unname()) |> filter(seqnames %in% c('chr1', 'chr2', 'PAR', 'XSR', 'YSR')) ) + aes(x=start, y=dNdS_PRANK.median) + geom_point() + facet_wrap(~seqnames) + geom_smooth()
```
...
chrw_rep <- SimpleList()
chrw_rep$Oki <- grWindows(genome=genomes$Oki, meta=reps$Oki, windowSize=windowSize, meta_prefix = 'repeats.')
chrw_rep$Oki <- flagLongShort(chrw_rep$Oki, longShort$OKI2018.I69)

chrw_rep$Osa <- grWindows(genome=genomes$Osa, meta=reps$Osa, windowSize=windowSize, meta_prefix = 'repeats.')
chrw_rep$Osa <- flagLongShort(chrw_rep$Osa, longShort$OSKA2016v1.9)

chrw_rep$Bar <- grWindows(genome=genomes$Bar, meta=reps$Bar, windowSize=windowSize, meta_prefix = 'repeats.')
chrw_rep$Bar <- flagLongShort(chrw_rep$Bar, longShort$Bar2.p4)
...
ggplot(as.data.frame(chrw_rep$Oki) |> filter(seqnames %in% c('chr1', 'chr2', 'PAR', 'XSR', 'YSR')) ) + aes(x=start, y=repeats.Class.count.total) + geom_point() + facet_wrap(Arm~seqnames) + geom_smooth() + scale_y_log10()

ggplot(as.data.frame(chrw_rep$Osa) |> filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) ) + aes(x=start, y=repeats.Class.count.total) + geom_point() + facet_wrap(Arm~seqnames) + geom_smooth() + scale_y_log10()

ggplot(as.data.frame(chrw_rep$Bar) |> filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) ) + aes(x=start, y=repeats.Class.count.total) + geom_point() + facet_wrap(Arm~seqnames) + geom_smooth() + scale_y_log10()
```
