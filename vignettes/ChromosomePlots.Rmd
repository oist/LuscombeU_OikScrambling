...
```{r chromosome_plots_transcripts_bar, dependson="chromosome_plots_transcripts", fig.height=6, fig.width=6, message=FALSE, warning=FALSE, dev=c("svg", "png")}
plots$Bar <- list()
...
mcols(chrw$Bar) <- cbind(mcols(chrw$Bar), mcols(grWindows(genome=genomes$Bar, meta=operons$Bar, windowSize=windowSize, meta_prefix = 'operons.'))) |> suppressWarnings()
```
...
```{r chromosome_plots_operons_osa, dependson="chromosome_plots_transcripts", fig.height=6, fig.width=8, message=FALSE, warning=FALSE, dev=c("svg", "png")}
...
nTiles <- 100
tiles_Oki <- genomes$Oki |> BSgenomeToGR() |> genomeGRToTiles(nTiles) |> plyranges::filter(seqnames %in% c("chr1", "chr2", "PAR", "XSR", "YSR"))
tiles_Osa <- genomes$Osa |> BSgenomeToGR() |> genomeGRToTiles(nTiles) |> plyranges::filter(seqnames %in% c("Chr1", "Chr2", "PAR", "XSR", "YSR"))
tiles_Bar <- genomes$Bar |> BSgenomeToGR() |> genomeGRToTiles(nTiles) |> plyranges::filter(seqnames %in% c("Chr1", "Chr2", "PAR", "XSR", "YSR"))
...
## Chromosome tiles: edges of arms vs. centres
```{r chromosome_arm_edges}
...
flag_chromosome_percentiles <- function(chrt, percentile=0.1) {
  ...
  edge_annotations <- endoapply(gr_list, function(gr){
    ...
    gr$chr.edge[cumulative_percentile<percentile] <- TRUE
    gr$chr.edge[cumulative_percentile>(1-percentile)] <- TRUE
...
chrt_unified <- flag_chromosome_percentiles(chrt_unified)
...
flag_chromosome_arm_percentiles <- function(chrt, percentile=0.1, arm="Oki.Arm") {
  ...
  edge_annotations <- endoapply(gr_list, function(gr){
    gr$chr.arm.edge <- FALSE
    total_length <- gr |> width() |> sum()
    length_percentile <- end(gr)/total_length
    cumulative_length <- cumsum(gr |> width())
    cumulative_percentile <- cumulative_length/total_length

    gr$chr.arm.edge[cumulative_percentile<percentile] <- TRUE
    gr$chr.arm.edge[cumulative_percentile>(1-percentile)] <- TRUE
...
chrt_unified <- flag_chromosome_arm_percentiles(chrt_unified)
```
