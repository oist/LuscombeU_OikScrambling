  ...
  for (name in colnames(mcols(meta))) {
    ...
    if(is.numeric(data)) {
      ...
      if(length(levels(data)) > 1) {
        ...
        colnames(binned_DataFrame) <- paste0(meta_prefix, colnames(binned_DataFrame), ".count.total")
...
```{r chromosome_plots_2, dependson="chromosome_plot_helper_fxns", fig.height=6, fig.width=12, message=FALSE, warning=FALSE, dev=c("svg", "png")}
repeat_dist <- SimpleList()
repeat_dist$Oki <- as.data.frame(chrw$Oki) |> as_tibble() |> group_by(seqnames, Arm) |> summarize(repeats.Class.count.table)
