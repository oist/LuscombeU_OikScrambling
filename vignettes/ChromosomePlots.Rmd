...
ggboxplot(tb, x='seqnames_arm', y='breakpoints.count.mean', color='seqnames', add='jitter') +
  theme(axis.text.x=element_text(angle=90)) +
  ...
  ggtitle('Tiles (averages across Oki, Osa, Bar): Breakpoints per chr+arm')

ggboxplot(tb, x='seqnames_arm', y='transcripts.dNdS.mean', color='seqnames', add='jitter') +
  theme(axis.text.x=element_text(angle=90)) +
  ylab('dN/dS per tile') +
  stat_compare_means(comparison=list(c('Chr1_short', 'Chr1_long'), c('Chr2_short', 'Chr2_long'), c('PAR_short', 'PAR_long'), c('XSR_XSR', 'PAR_short'), c('XSR_XSR', 'PAR_long')), paired = FALSE, na.rm = TRUE, method='t.test')  + 
  ggtitle('Tiles (averages across Oki, Osa, Bar): dNdS per chr+arm')

# And a correlation plot
ggplot(tb) + aes(x=breakpoints.count.mean, y=transcripts.dNdS.mean) + geom_point() + geom_smooth(method='lm', col='red') + ggpubr::stat_cor() + facet_wrap(seqnames~Oki.Arm, nrow=1, scales = 'free') + ggtitle('Tiles (averages across Oki, Osa, Bar): dNdS per chr+arm') + ylab('dN/dS')
