...
We can produce similar objects, but from "core" orthogroups, that have a member
in each species of the tunicate clade.
...
present as a single copy in every genome.
...
sapply(orthoBlocks_one2oneInOiks, \(gb) sum(gb$geneNumber))
```
...
barBreaks <- get_bps(orthoBlocks_one2oneInOiks$Oki_Bar, direction = "mid")

# Stringent list of ROIs based on the one2oneInOiks objects
(ROIs <- orthoBlocks_one2oneInOiks$Oki_Osa |>
           reIndex() |>
           subsetByOverlaps(barBreaks) |>
           subset(geneNumber > 2))
...
(GoI1 <- head(ROIs$geneNames, 1) |> unlist())
...
```{r}
### Between "g1207.t1" and "g1208.t1", there is a repeat region, where the Bar genome broke, but that is still in the Osa genome (unalMap)
...
(BP1_Oki_Osa <- unalMap$Oki_Osa |> subsetByOverlaps(GRanges("chr1", (4577337 + 4577995)/2))) # between g1207.t1 and g1208.t1 on Oki
...
oP_one2oneInOiks_bridges$Oki_Bar |> subsetByOverlaps(upstreamOfg1208.t1)
