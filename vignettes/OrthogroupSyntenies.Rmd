...
coalOrtho <- function(gb) {
  ...
  coal$orthoFlagDF <- CharacterList(split(gb$orthoFlagDF [subjectHits(ov)], queryHits(ov))) |> unname()
...
```{r expand_contract_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
...
ggplot( dat ) +
  ...
  geom_bar(stat='count') +
  facet_wrap(~Arm) +
  xlab('Present in all spp.') +
  ggtitle("Oki-Osa: Location of HOGs present in all spp.")
```

```{r expand_contract_2, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("chr1","chr2","PAR","XSR","YSR")
...
ggplot( dat ) +
  ...
  facet_wrap(~Arm) +
  xlab('Present in all spp.') +
  ggtitle("Oki-Bar: Location of HOGs present in all spp.")
```

```{r expand_contract_3, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("Chr1","Chr2","PAR","XSR","YSR")
...
ggplot( dat ) +
  ...
  facet_wrap(~Arm) +
  xlab('Present in all spp.') +
  ggtitle("Osa-Bar: Location of HOGs present in all spp.")
```

It seems that, in general, a greater fraction of genes on long arms can be found across all species. Let us continue this logic further.

Where are one-to-one orthologues found?

```{r expand_contract_4, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("chr1","chr2","PAR","XSR","YSR")
dat = subset(as.data.frame(orthoPairs$Oki_Osa), as.data.frame(orthoPairs$Oki_Osa)$seqnames %in% big_chr)
ggplot( dat ) +
  ...
  facet_wrap(~Arm) +
  xlab('One-to-one HOG') +
  ggtitle("Oki-Osa: Location of one-to-one HOGs")
```

```{r expand_contract_5, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("chr1","chr2","PAR","XSR","YSR")
dat = subset(as.data.frame(orthoPairs$Oki_Bar), as.data.frame(orthoPairs$Oki_Bar)$seqnames %in% big_chr)
ggplot( dat ) +
  aes(x=unlist(lapply(orthoFlagDF, function(x) x$one_to_one))) +
  ...
  facet_wrap(~Arm) +
  xlab('One-to-one HOG') +
  ggtitle("Oki-Bar: Location of one-to-one HOGs")
```

```{r expand_contract_6, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("Chr1","Chr2","PAR","XSR","YSR")
dat = subset(as.data.frame(orthoPairs$Osa_Bar), as.data.frame(orthoPairs$Osa_Bar)$seqnames %in% big_chr)
ggplot( dat ) +
  aes(x=unlist(lapply(orthoFlagDF, function(x) x$one_to_one))) +
  ...
  facet_wrap(~Arm) +
  xlab('One-to-one HOG') +
  ggtitle("Osa-Bar: Location of one-to-one HOGs")
```

So, it also seems that one-to-one genes, as a proportion, are less common in short arms.
