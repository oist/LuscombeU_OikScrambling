...
p1 <- orthoPairs$Oki_Osa |> plyranges::filter(seqnames %in% c('chr1', 'chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> plyranges::filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> makeOxfordPlots(type='point', diag = F) + theme_bw() + theme(legend.position='none') + scale_color_manual(values = c("#E78AC3", "#A6D854", "#FFD92F", "#8DA0CB", "#66C2A5")) + ggtitle(NULL)
```

```{r orthopair_plot_Bar_Osa}
p2 <- orthoPairs$Bar_Osa |> plyranges::filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> plyranges::filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> makeOxfordPlots(type='point', diag=F) + theme_bw() + theme(legend.position='none') + scale_color_manual(values = c("#E78AC3", "#A6D854", "#FFD92F", "#8DA0CB", "#66C2A5")) + ggtitle(NULL)
```

```{r orthopair_plot_Aom_Osa}
p3 <- orthoPairs$Osa_Aom |>
  plyranges::filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) |>
  swap() |>
  scaffoldByFlipAndMerge(scafs$Aom_Osa, drop = TRUE) |>
...
(p4 <- orthoPairs$Bar_Oki |> plyranges::filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> plyranges::filter(seqnames %in% c('chr1', 'chr2', 'PAR', 'XSR', 'YSR')) |> swap() |> makeOxfordPlots(type='point', diag = F) + theme_bw() + theme(legend.position='none') + scale_color_manual(values = c("#E78AC3", "#A6D854", "#FFD92F", "#8DA0CB", "#66C2A5")) + ggtitle(NULL))
```

```{r orthopair_plot_Oki_Kum, fig.height=8, fig.width=8,  dev=c("svg", "pdf", "png"), fig.ext=c("svg", "pdf", "png")}
(p5 <- orthoPairs$Oki_Kum |> plyranges::filter(seqnames %in% c('chr1', 'chr2', 'PAR', 'XSR', 'YSR')) |> swap()  |> makeOxfordPlots(type='point', diag=T) + theme_bw() + theme(legend.position='none')  + ggtitle(NULL))
```
...
```{r synteny_block_histograms, fig.width=8, fig.height=6, dev=c("svg", "pdf", "png"), fig.ext=c("svg", "pdf", "png")}
...
numeric_vector_to_summary_df <- function(vec, name=NULL, digits=2) {
  summary_df <- summary(as.numeric(vec)) |> unclass() |>  as.data.frame() |> tibble::rownames_to_column("stat") |> dplyr::rename(value=2)
...
summary_df_to_geom_data <- function(summary_df, size=c(0.4, 0.2, 0.4, 0.4, 0.2, 0.4), lty=c(1, 3, 1, 1, 3, 1), col=c("#595959", "darkgray", "darkred", "darkred", "darkgray", "#595959")) {
...
gg_stat_hist <- function(df, plot.what=NULL, include_stats=c("median", "max"), size=c(0.5, 0.3, 0.5, 0.4, 0.3, 0.5), lty=c(1, 3, 1, 1, 3, 1), col=c("#595959", "darkgray", "darkred", "darkred", "darkgray", "#595959"), bins=30, include_lines = TRUE, include_text=TRUE, digits=2, ...) {
...
# Close
p1 <- orthoBlocks$Oki_Kum |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Kum: synteny block size distribution")
p2 <- orthoBlocks$Osa_Aom |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Aom: synteny block size distribution")
p3 <- orthoBlocks$Bar_Nor |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Bar-Nor: synteny block size distribution")
# Intermediate
p4 <- orthoBlocks$Osa_Bar |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Bar: synteny block size distribution")
p5 <- orthoBlocks$Osa_Nor |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Nor: synteny block size distribution")
p6 <- orthoBlocks$Bar_Aom |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Bar-Aom: synteny block size distribution")
# Distant
p7 <- orthoBlocks$Oki_Osa |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Osa: synteny block size distribution")
p8 <- orthoBlocks$Oki_Bar |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Bar: synteny block size distribution")
p9 <- orthoBlocks$Osa_Kum |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Kum: synteny block size distribution")
...
p9
(p1 + p2 + p3) # Only close
(p4 + p5 + p6) # Only intermediate
(p7 + p8 + p9) # Only distant
(p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9) # All plots
(p7 + p4 + p2) # By increasing phylogenetic distance from Osaka

# Just Okinawa plots
p10 <- orthoBlocks$Oki_Osa |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Osa: synteny block size distribution")
p11 <- orthoBlocks$Oki_Bar |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Bar: synteny block size distribution")
p12 <- orthoBlocks$Oki_Aom |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Aom: synteny block size distribution")
p13 <- orthoBlocks$Oki_Nor |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Nor: synteny block size distribution")
p14 <- orthoBlocks$Oki_Kum |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Kum: synteny block size distribution")

(p10 + p11 + p12 + p13 + p14)
```

```{r synteny_block_histograms_2, fig.width=8, fig.height=6, dev=c("svg", "pdf", "png"), fig.ext=c("svg", "pdf", "png")}
# Transformed on X axis to simplify things.
# Close
p1 <- orthoBlocks$Oki_Kum |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Kum: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p2 <- orthoBlocks$Osa_Aom |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Aom: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p3 <- orthoBlocks$Bar_Nor |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Bar-Nor: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
# Intermediate
p4 <- orthoBlocks$Osa_Bar |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Bar: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p5 <- orthoBlocks$Osa_Nor |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Nor: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p6 <- orthoBlocks$Bar_Aom |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Bar-Aom: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
# Distant
p7 <- orthoBlocks$Oki_Osa |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Osa: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p8 <- orthoBlocks$Oki_Bar |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Oki-Bar: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
p9 <- orthoBlocks$Osa_Kum |> as.data.frame() |> gg_stat_hist(plot.what="geneNumber", lty=3) + xlab('Number of genes') + ylab('Count') + ggtitle("Osa-Kum: synteny block size distribution") + scale_x_continuous(trans="log10", limits=c(NA, 1000))
...
```{r synteny_block_histograms_3, fig.width=8, fig.height=6, dev=c("svg", "pdf", "png"), fig.ext=c("svg", "pdf", "png")}}
