...
collect_clade_expansions <- function(orthopairs, sp1, sp2, big_chr) {
  result <- SimpleList()
  orthopair <- orthopairs[[paste(sp1, sp2, sep="_")]]
  orthopair$midpoints <- grMidpoint(orthopair)
  orthopair <- as.data.frame(orthopair)
  df <- subset(orthopair, orthopair$seqnames %in% big_chr)
  # Identify clade-specific expansions
  dat_clade <- df[as.logical(unlist(lapply(df$orthoFlagDF, function(x) x[[paste("expansion_clade_", sp1, "_", sp2, sep="")]] )))==TRUE,] |> as.data.frame()
  ...
  df_sp1 <- subset(df_sp1, df_sp1$seqnames %in% big_chr)
  dat_sp1 <- df_sp1[which(unlist(lapply(df_sp1$orthoFlagDF, function(x) x$expansion_species_specific ))==sp1),]
  result[[sp1]] <- dat_sp1
  # TODO
  # sp2 is trickier.
  ...
  df_sp2 <- as.data.frame(transcripts[[sp2]])
  
  result
}
dat <- collect_clade_expansions(orthopairs=orthoPairs, sp1="Oki", sp2="Kum", big_chr=big_chr)
```
```
ggplot(dat) +
  aes(x=midpoints) +
