...
# Use the flagOrthogroupHomologies function to identify different kinds of homologies.
# This includes species-specific expansions (i.e., a HOG that contains one species)
# or specific-specific deletions (i.e., a HOG in all species but one).
# The clade list makes it possible to identify more fine-grained groupings.
orthoflags = do.call(rbind, flagOrthogroupHomologies(system.file("extdata/OrthoFinder/N19.tsv", package = "BreakpointsData"), clades=clades, species_name_map=species_name_map))
#orthoflags = do.call(rbind, OikScrambling:::flagOrthogroupHomologies(system.file("extdata/OrthoFinder/N19.tsv", package = "BreakpointsData"), clades=clades, species_name_map=species_name_map))
# Note that the function produces warnings when more than one clade is satisfied
# by the counts. You can observe these with warnings().
# warnings()
```

```{r cleaning_orthoflags}
rownames(orthoflags) <- NULL
(head(orthoflags))
```

The `orthoflags` object yields a data frame with one row per orthogroup, the other columns of the HOG table, as well as a set of orthogroup annotations based on the counts of every species and clade. 

```{r flagging_orthogroups_2}
# Take a look at the distribution of different species flags. 
ggplot(orthoflags) +
    aes(x=reorder(flag, flag, function(x) length(x))) + geom_bar(stat="count", position="dodge") +
    facet_wrap(~ present_in_all_species, scales = 'free' ) +
    xlab(NULL) +
    coord_flip()
```
...
# This function takes the output of `flagOrthogroupHomologies` and parses it into a data frame
