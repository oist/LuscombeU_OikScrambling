...
sapply(orthoPairs[1:15], \(gb) {
  x <- gb |> dplyr::filter(HOG == "N19.HOG0000192")
  if (length(x) == 0) return(NULL)
  x})

Hox4 <- orthoPairs$Oki_Osa |> dplyr::filter(HOG == "N19.HOG0000192")
...
```{r Hox1}
sapply(orthoPairs[1:15], \(gb) {
  x <- gb |> dplyr::filter(HOG == "N19.HOG0000193")
  if (length(x) == 0) return(NULL)
  x})

Hox1 <- orthoPairs$Oki_Osa |> dplyr::filter(HOG == "N19.HOG0000193")
