...
Where are one-to-one orthologues found?
...
As a point of clarification, a HOG is either exactly one-to-one (exactly one HOG participant from every species) or not (meaning a gene has been duplicated somewhere and has not been fully resolved into clade-specific HOGs, or else the gene is missing in one or more species).
...
Common-gene duplications
------------------------
Genes that are found in all species, and which have seemingly been duplicated (>1 in any species), occur where across the genome?

```{r species_expansion_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
big_chr = c("chr1","chr2","PAR","XSR","YSR")
arm_df = as.data.frame(longShort$Oki)
arm_df$chr_arm = paste(arm_df$seqnames, "_", arm_df$Type, sep="")
transcript_df = as.data.frame(transcripts$Oki)

arm_df$geneNumber <- apply(arm_df, 1, function(region){
  region = as.list(region)
  gene_count = nrow(subset(transcript_df, transcript_df$seqnames == region$seqnames & as.numeric(transcript_df$end) <= as.numeric(region$end) & as.numeric(transcript_df$start) >= as.numeric(region$start) ))
  gene_count
})

dat =  subset(as.data.frame(orthoPairs$Oki_Osa), as.data.frame(orthoPairs$Oki_Osa)$seqnames %in% big_chr)
dat = dat[unlist(lapply(dat$orthoFlagDF, function(x) x$common_gene_duplication ==TRUE)) , ]
dat = dat |> dplyr::group_by(seqnames, Arm) |> dplyr::summarise(count=dplyr::n())

dat$count_per_gene = apply(dat, 1, function(region){
  gene_number = arm_df[arm_df$chr_arm==paste(region[["seqnames"]], region[["Arm"]], sep="_"),]$geneNumber
  mod_count = as.numeric(region[["count"]])/gene_number
  mod_count
})

dat$count_per_Mbp = apply(dat, 1, function(region){
  n_mbp = arm_df[arm_df$chr_arm==paste(region[["seqnames"]], region[["Arm"]], sep="_"),]$width/1000000
  mod_count = as.numeric(region[["count"]])/n_mbp
  mod_count
})

ggplot(dat) +
  aes(x=seqnames, y=count) +
...
collect_clade_expansions <- function(orthopairs, sp1, sp2, bigChr, longShort) {
  result <- SimpleList()
  ...
  # Obtain species-specific expansions for both species
  # sp1 must be a chromosomal assembly - and getting its gene coordinates relative to the
  # sp1 assembly is therefore really easy.
  df_sp1 <- as.data.frame(transcripts[[sp1]])
  df_sp1$midpoint <- grMidpoint(transcripts[[sp1]])
  ...
  dat_sp1 <- flagLongShort(GRanges(df_sp1), longShort) |> as.data.frame()
  result[[sp1]] <- dat_sp1

  # sp2 is trickier.
  # Its species-specific expansions cannot be part of the orthoPairs, by definition.
  # So what you can do is guess where that transcript corresponds to on the reference
  # genome based on its transcript coordinates, but this is tricky...
  ...
  df_sp2_specific <- lapply(1:nrow(df_sp2), function(gene){
    ...
    if(nrow(prev_gene)!=0 & nrow(next_gene)!=0) {
      contig_names <- c(as.character(cur_gene$seqnames), as.character(prev_gene$seqnames),  as.character(next_gene$seqnames))
      # If all genes on same contig...
      if(length(unique(contig_names)) == 1) { 
        ...
        if(prev_gene$tx_name %in% orthopair$query.tx_name & next_gene$tx_name %in% orthopair$query.tx_name){
          # Return a new orthopair that does NOT exist in sp1 query genome.
          # The range is somewhere between the start and end sites of the surrounding genes.
          ...
          mod_orthopair$start   <- prev_orthopair$end+1
          mod_orthopair$end     <- next_orthopair$start-1
          mod_orthopair$width   <- next_orthopair$start - prev_orthopair$end
          ...
          mod_orthopair$dNdS_HmmCleaner <- NA # they are only computed on one-to-one genes.
          ...
          mod_orthopair$query.dNdS_HmmCleaner <- NA # they are only computed on one-to-one genes.
          ...
          mod_orthopair$midpoint <- cur_gene$midpoint
        ...
        warning_message = paste("sp2-specific gene ", cur_gene$tx_name, "'s next gene ", next_gene$tx_name, " and previous gene ", prev_gene$tx_name, " are not on the same contig. Contig names: ", paste(contig_names, collapse=","), "\n Can't infer position of a species-specific gene unless neighbouring genes are on same contig.",  sep="")
        warning(warning_message)
        return(NULL)
      }
    } else {
...
dat <- collect_clade_expansions(orthopairs=orthoPairs, sp1="Oki", sp2="Kum", bigChr=big_chr, longShort=longShort$Oki)
```
```{r hello2}
ggplot(dat$Oki_Kum) +
  aes(x=midpoint) +
  ...
  ggtitle('Gene family expansions: Oki-Kum')
...
ggplot(dat$Oki) +
  ...
  #geom_boxplot() +
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Gene family expansions: Oki-specific')
...
```
apply(head(as.data.frame(orthoBlocks$Oki_Osa),n=100), 1, function(x) table(unlist(lapply(x$orthoFlagDF, function(y) y$present_in_all_spp))))
```
