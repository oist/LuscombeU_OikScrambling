...
---------------------------------------------------------------
In the following section, I attempt to identify species- and clade-specific gene family expansions using counts from HOG counts. For a given `query`-`target` combination, the `collect_clade_expansions()` function extracts from a `GBreaks` object 3 different data frames:

1. Species-specific expansions in the query genome (referred to as `sp1` in the code below)
2. Species-specific expansions in the target genome (referred to as `sp2` in the code below)
3. Clade-specific expansions common to the query and target
...
collectCladeExpansions <- function(orthopairs, sp1, sp2, bigChr, longShort) {
  ...
  # sp1, the chromosomal assembly, is somewhat easy to indicate positions of species-specific expansions.
  ...
  # Subset transcripts to include only the longest per gene: split by gene ID, sort by longest, glue back together
  if(sp2 == "Nor") { # catch Norway exception
    transcripts_sp2$geneid <- unname(sapply(transcripts_sp2$tx_name, function(s) s ))
  } else {
        ...
        if(prev_gene$tx_name %in% orthopair$query.tx_name & next_gene$tx_name %in% orthopair$query.tx_name & nrow(orthopair[orthopair$query.tx_name == prev_gene$tx_name,])!=0 & nrow(orthopair[orthopair$query.tx_name == next_gene$tx_name,]) != 0 ) {
          ...
          if(length(unique(query_seqnames)) == 1 & nrow(target_prev_gene)!=0 & nrow(target_next_gene)!=0 ) {
            ...
            if( (target_next_gene_index-target_prev_gene_index)==2) { # If target's genes are collinear...
              ...
              mod_orthopair <- mod_orthopair[,!grepl("OG" , colnames(mod_orthopair))] # Drop HOG and OG columns to facilitate rbinding
...
expansions$Oki_Kum <- collectCladeExpansions(orthopairs=orthoPairs, sp1="Oki", sp2="Kum", bigChr=big_chr, longShort=longShort$Oki)
big_chr <- c("Chr1","Chr2","PAR","XSR","YSR")
expansions$Osa_Aom <- collectCladeExpansions(orthopairs=orthoPairs, sp1="Osa", sp2="Aom", bigChr=big_chr, longShort=longShort$Osa)
expansions$Bar_Nor <- collectCladeExpansions(orthopairs=orthoPairs, sp1="Bar", sp2="Nor", bigChr=big_chr, longShort=longShort$Bar)
```
...
------- | ------- | ------- | -------
1 (ends of query contigs) | 11 | 1 | 100
2 (query genome orthologue neighbours not in orthoPairs) | 919 | 733 | 1842
3 (query genome orthologue neighbours not on same query chr.) | 26 | 17 | 64
3 (query genome orthologue neighbours not collinear on query chr.) | 286 | 307 | 490
...
The `expansions` object contains data frames that tabulate the number of clade- and species-specific HOG expansions. So, given this, where do the species-specific HOG expansions land across the chromosome?

Gene family expansions: Okinawa-Kume
------------------------------------

```{r gene_expansions_pos_oki_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsHist <- function(expansionsDF, facet.by="seqnames~Arm", binWidth=100000) {
  ggplot(expansionsDF) +
    aes(x=midpoint) +
...
binWidth=100000
# Oki-specific gene family expansions
plotExpansionsHist(expansions$Oki_Kum$Oki, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Oki: Species-specific gene family expansions")
plotExpansionsHist(expansions$Oki_Kum$Oki, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Oki: Species-specific gene family expansions")

# Kum-specific gene family expansions
plotExpansionsHist(expansions$Oki_Kum$Kum, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Kum: Species-specific gene family expansions")
plotExpansionsHist(expansions$Oki_Kum$Kum, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Kum: Species-specific gene family expansions")
```

```{r gene_expansions_pos_oki_2, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsBox <- function(duplicationDF, facet.by="seqnames~Arm", binWidth=100000) {
  df <- duplicationDF |> mutate(bin=cut_width(midpoint, binWidth)) |> dplyr::group_by(seqnames, Arm, bin) |> dplyr::summarise(count=dplyr::n())
  ggplot(df) + 
    aes(y=count) +
    geom_boxplot() +
    facet_wrap(facet.by, ncol=2)
}

# Oki-specific
plotExpansionsBox(expansions$Oki_Kum$Oki, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Oki: Species-specific gene family expansions")
plotExpansionsBox(expansions$Oki_Kum$Oki, facet.by='~Arm', binWidth=binWidth) + ggtitle("Oki: Species-specific gene family expansions")
# Kum-specific
plotExpansionsBox(expansions$Oki_Kum$Kum, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Kum: Species-specific gene family expansions")
plotExpansionsBox(expansions$Oki_Kum$Kum, facet.by='~Arm', binWidth=binWidth) + ggtitle("Kum: Species-specific gene family expansions")
```

It seems there _might_ be some relationship between gene duplication and chromosomes or chromosome arms. But this is not made clear by the histograms or boxplots.

The question is, are there statistically significant differences in the occurrence of gene expansions between chromosomes and chromosome arms?

```{r gene_expansions_pos_oki_3, fig.height=4, fig.width=6, dev=c("svg", "png")}
calcExpansionStats <- function(duplicationDF, binWidth=100000, conf.level=0.95) {
  df <- duplicationDF |> mutate(bin=cut_width(midpoint, binWidth)) |> dplyr::group_by(seqnames, Arm, bin) |> dplyr::summarise(count=dplyr::n())
...
(calcExpansionStats(expansions$Oki_Kum$Oki, conf.level=0.95, binWidth=binWidth)$anova)
(calcExpansionStats(expansions$Oki_Kum$Kum, conf.level=0.95, binWidth=binWidth)$anova)
```

And the answer is - at least for specific-specific genes between Okinawa and Kume - maybe?

We can ask the same questions about clade-specific expansions (shared by Oki AND Kum) or the sum of species-specific expansions (Oki OR Kum).

```{r gene_expansions_pos_oki_4, fig.height=6, fig.width=4, dev=c("svg", "png")}
# Oki-specific gene family expansions
plotExpansionsHist(expansions$Oki_Kum$Oki_Kum, facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Oki-Kum: Clade-specific gene family expansions')
plotExpansionsHist(expansions$Oki_Kum$Oki_Kum, facet.by="~Arm", binWidth=binWidth) + ggtitle('Oki-Kum: Clade-specific gene family expansions')
plotExpansionsBox(expansions$Oki_Kum$Oki_Kum, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Oki-Kum: Clade-specific gene family expansions")
plotExpansionsBox(expansions$Oki_Kum$Oki_Kum, facet.by='~Arm', binWidth=binWidth) + ggtitle("Oki-Kum: Clade-specific gene family expansions")
(calcExpansionStats(expansions$Oki_Kum$Oki_Kum, conf.level=0.95, binWidth=binWidth)$anova)
```

```{r gene_expansions_pos_oki_5, fig.height=6, fig.width=4, dev=c("svg", "png")}
# Oki-specific gene family expansions
plotExpansionsHist(rbind(expansions$Oki_Kum$Oki, expansions$Oki_Kum$Kum), facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Oki OR Kum: Species-specific gene family expansions')
plotExpansionsHist(rbind(expansions$Oki_Kum$Oki, expansions$Oki_Kum$Kum), facet.by="~Arm", binWidth=binWidth) + ggtitle('Oki OR Kum: Species-specific gene family expansions')
plotExpansionsBox(rbind(expansions$Oki_Kum$Oki, expansions$Oki_Kum$Kum), facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Oki OR Kum: Species-specific gene family expansions")
plotExpansionsBox(rbind(expansions$Oki_Kum$Oki, expansions$Oki_Kum$Kum), facet.by='~Arm', binWidth=binWidth) + ggtitle("Oki OR Kum: Species-specific gene family expansions")
(calcExpansionStats(rbind(expansions$Oki_Kum$Oki, expansions$Oki_Kum$Kum), conf.level=0.95, binWidth=binWidth)$anova)
```

Gene family expansions: Osaka-Aomori
------------------------------------

```{r gene_expansions_pos_Osa_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsHist(expansions$Osa_Aom$Osa, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Osa: Species-specific gene family expansions")
plotExpansionsHist(expansions$Osa_Aom$Osa, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Osa: Species-specific gene family expansions")

plotExpansionsHist(expansions$Osa_Aom$Aom, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Aom: Species-specific gene family expansions")
plotExpansionsHist(expansions$Osa_Aom$Aom, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Aom: Species-specific gene family expansions")
```

```{r gene_expansions_pos_Osa_2, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsBox(expansions$Osa_Aom$Osa, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Osa: Species-specific gene family expansions")
plotExpansionsBox(expansions$Osa_Aom$Osa, facet.by='~Arm', binWidth=binWidth) + ggtitle("Osa: Species-specific gene family expansions")

plotExpansionsBox(expansions$Osa_Aom$Aom, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Aom: Species-specific gene family expansions")
plotExpansionsBox(expansions$Osa_Aom$Aom, facet.by='~Arm', binWidth=binWidth) + ggtitle("Aom: Species-specific gene family expansions")
```

```{r gene_expansions_pos_Osa_3, fig.height=4, fig.width=6, dev=c("svg", "png")}
(calcExpansionStats(expansions$Osa_Aom$Osa, conf.level=0.95, binWidth=binWidth)$anova)
(calcExpansionStats(expansions$Osa_Aom$Aom, conf.level=0.95, binWidth=binWidth)$anova)
```

```{r gene_expansions_pos_Osa_4, fig.height=6, fig.width=4, dev=c("svg", "png")}
plotExpansionsHist(expansions$Osa_Aom$Osa_Aom, facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Osa-Aom: Clade-specific gene family expansions')
plotExpansionsHist(expansions$Osa_Aom$Osa_Aom, facet.by="~Arm", binWidth=binWidth) + ggtitle('Osa-Aom: Clade-specific gene family expansions')

plotExpansionsBox(expansions$Osa_Aom$Osa_Aom, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Osa-Aom: Clade-specific gene family expansions")
plotExpansionsBox(expansions$Osa_Aom$Osa_Aom, facet.by='~Arm', binWidth=binWidth) + ggtitle("Osa-Aom: Clade-specific gene family expansions")
(calcExpansionStats(expansions$Osa_Aom$Osa_Aom, conf.level=0.95, binWidth=binWidth)$anova)
```

```{r gene_expansions_pos_Osa_5, fig.height=6, fig.width=4, dev=c("svg", "png")}
plotExpansionsHist(rbind(expansions$Osa_Aom$Osa, expansions$Osa_Aom$Aom), facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Osa OR Aom: Species-specific gene family expansions')
plotExpansionsHist(rbind(expansions$Osa_Aom$Osa, expansions$Osa_Aom$Aom), facet.by="~Arm", binWidth=binWidth) + ggtitle('Osa OR Aom: Species-specific gene family expansions')

plotExpansionsBox(rbind(expansions$Osa_Aom$Osa, expansions$Osa_Aom$Aom), facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Osa OR Aom: Species-specific gene family expansions")
plotExpansionsBox(rbind(expansions$Osa_Aom$Osa, expansions$Osa_Aom$Aom), facet.by='~Arm', binWidth=binWidth) + ggtitle("Osa OR Aom: Species-specific gene family expansions")

(calcExpansionStats(rbind(expansions$Osa_Aom$Osa, expansions$Osa_Aom$Aom), conf.level=0.95, binWidth=binWidth)$anova)
```

Gene family expansions: Barcelona-Norway
----------------------------------------

```{r gene_expansions_pos_Bar_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsHist(expansions$Bar_Nor$Bar, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Bar: Species-specific gene family expansions")
plotExpansionsHist(expansions$Bar_Nor$Bar, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Bar: Species-specific gene family expansions")

plotExpansionsHist(expansions$Bar_Nor$Nor, facet.by="seqnames~Arm", binWidth=binWidth) +
  ggtitle("Nor: Species-specific gene family expansions")
plotExpansionsHist(expansions$Bar_Nor$Nor, facet.by="~Arm", binWidth = binWidth) +
  ggtitle("Nor: Species-specific gene family expansions")
```

```{r gene_expansions_pos_Bar_2, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotExpansionsBox(expansions$Bar_Nor$Bar, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Bar: Species-specific gene family expansions")
plotExpansionsBox(expansions$Bar_Nor$Bar, facet.by='~Arm', binWidth=binWidth) + ggtitle("Bar: Species-specific gene family expansions")

plotExpansionsBox(expansions$Bar_Nor$Nor, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Nor: Species-specific gene family expansions")
plotExpansionsBox(expansions$Bar_Nor$Nor, facet.by='~Arm', binWidth=binWidth) + ggtitle("Nor: Species-specific gene family expansions")
```

```{r gene_expansions_pos_Bar_3, fig.height=4, fig.width=6, dev=c("svg", "png")}
(calcExpansionStats(expansions$Bar_Nor$Bar, conf.level=0.95, binWidth=binWidth)$anova)
(calcExpansionStats(expansions$Bar_Nor$Nor, conf.level=0.95, binWidth=binWidth)$anova)
```

```{r gene_expansions_pos_Bar_4, fig.height=6, fig.width=4, dev=c("svg", "png")}
plotExpansionsHist(expansions$Bar_Nor$Bar_Nor, facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Bar-Nor: Clade-specific gene family expansions')
plotExpansionsHist(expansions$Bar_Nor$Bar_Nor, facet.by="~Arm", binWidth=binWidth) + ggtitle('Bar-Nor: Clade-specific gene family expansions')

plotExpansionsBox(expansions$Bar_Nor$Bar_Nor, facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Bar-Nor: Clade-specific gene family expansions")
plotExpansionsBox(expansions$Bar_Nor$Bar_Nor, facet.by='~Arm', binWidth=binWidth) + ggtitle("Bar-Nor: Clade-specific gene family expansions")
(calcExpansionStats(expansions$Bar_Nor$Bar_Nor, conf.level=0.95, binWidth=binWidth)$anova)
```

```{r gene_expansions_pos_Bar_5, fig.height=6, fig.width=4, dev=c("svg", "png")}
plotExpansionsHist(rbind(expansions$Bar_Nor$Bar, expansions$Bar_Nor$Nor), facet.by="seqnames~Arm", binWidth=binWidth) + ggtitle('Bar OR Nor: Species-specific gene family expansions')
plotExpansionsHist(rbind(expansions$Bar_Nor$Bar, expansions$Bar_Nor$Nor), facet.by="~Arm", binWidth=binWidth) + ggtitle('Bar OR Nor: Species-specific gene family expansions')

plotExpansionsBox(rbind(expansions$Bar_Nor$Bar, expansions$Bar_Nor$Nor), facet.by='seqnames~Arm', binWidth=binWidth) + ggtitle("Bar OR Nor: Species-specific gene family expansions")
plotExpansionsBox(rbind(expansions$Bar_Nor$Bar, expansions$Bar_Nor$Nor), facet.by='~Arm', binWidth=binWidth) + ggtitle("Bar OR Nor: Species-specific gene family expansions")

(calcExpansionStats(rbind(expansions$Bar_Nor$Bar, expansions$Bar_Nor$Nor), conf.level=0.95, binWidth=binWidth)$anova)
```

Species-specific genes
----------------------
The code above has to do with identifying gene families that have expanded specifically in one species or one clade. However, what might sometimes be interesting is to see

```{r species_specific_genes_oki}
speciesSpecificHOGPlot <- function(sp, bigChr, facet.by='seqnames~Arm') {
  speciesSpecificHOGs <- rownames(subset(as.data.frame(se_m), as.data.frame(se_m)$Oki > 0 & apply(as.data.frame(se_m)[,-4], 1, function(x) sum(x)==0)))
  speciesSpecificHOGsTranscripts <- rowData(se)[match(speciesSpecificHOGs, rowData(se)$HOG),]
  speciesSpecificHOGsTranscripts <- unlist(as.data.frame(speciesSpecificHOGsTranscripts$txLists)$Oki)
  speciesSpecificHOGsTranscripts <- lapply(speciesSpecificHOGsTranscripts, function(x) unlist(strsplit(x, '.v2.'))[2] ) |> unlist()
  df <- flagLongShort(transcripts$Oki, annot = longShort$Oki) 
  df$midpoint <- grMidpoint(df)
  df <- as.data.frame(df)
  df <- subset(df, df$tx_name %in% speciesSpecificHOGsTranscripts & df$seqnames %in% big_chr)
  ggplot(df) + 
    aes(x=midpoint) +
    geom_histogram() +
...
------------------------
This has some trickiness involved. 

```{r gene_oki_kum_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotOrthopairDeletionsBox <- function(orthoPair, facet.by="seqnames~Arm", bigChr=NULL, binWidth=100000) {
  orthoPair$midpoint <- grMidpoint(orthoPair)
  df <- as.data.frame(orthoPair)
  df <- subset(df, df$seqnames %in% bigChr)
  df <- df[which(!is.na(unlist(lapply(df$orthoFlagDF, function(x) x$deletion_species_specific)))),]
  df <- df |> mutate(bin=cut_width(midpoint, binWidth)) |> dplyr::group_by(seqnames, Arm, bin) |> dplyr::summarise(count=dplyr::n())
  ggplot(df) + 
    aes(y=count) +
    geom_boxplot() +
    facet_wrap(facet.by, ncol=2)
...
big_chr = c("chr1","chr2","PAR","XSR","YSR")
plotOrthopairDeletionsBox(orthoPairs$Oki_Kum, bigChr=big_chr)
```

```{r gene_oki_kum_2, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotOrthopairDeletionsHist <- function(orthoPair, facet.by="seqnames~Arm", bigChr=NULL, binWidth=100000) {
  orthoPair$midpoint <- grMidpoint(orthoPair)
  df <- as.data.frame(orthoPair)
  df <- subset(df, df$seqnames %in% bigChr)
  df <- df[which(!is.na(unlist(lapply(df$orthoFlagDF, function(x) x$deletion_species_specific)))),]
...
big_chr = c("chr1","chr2","PAR","XSR","YSR")
plotOrthopairDeletionsHist(orthoPairs$Oki_Kum, bigChr=big_chr)
```

```{r gene_oki_kum_3, fig.height=4, fig.width=6, dev=c("svg", "png")}
plotPartnerDeletionsHist <- function(sp1, sp2, count_matrix=NULL, facet.by="seqnames~Arm", bigChr=NULL, binWidth=100000) {
  # Process transcripts: add midpoint, subset to big chromosomes only, subset to only biggest genes
  transcripts_sp1 <- flagLongShort(transcripts[[sp1]], annot=longShort)
  transcripts_sp1$midpoint <- grMidpoint(transcripts_sp1)
  transcripts_sp1 <- subset(transcripts_sp1, transcripts_sp1$seqnames %in% bigChr)
  if(sp1 == "Nor") { # catch Norway exception
    transcripts_sp1$geneid <- unname(sapply(transcripts_sp1$tx_name, function(s) s ))
  } else {
    transcripts_sp1$geneid <- unname(sapply(transcripts_sp1$tx_name, function(s) unlist(strsplit(s, "\\.t"))[1] ))
  }
  ...
  # subset to cases where sp1 != 0 and sp2 = 0
  transcripts_sp1 <- lapply(transcripts_sp1, function(geneid) { # sort by longest
    biggest_gene = geneid[which.max(geneid$width),]
    biggest_gene
  })
...
big_chr = c("chr1","chr2","PAR","XSR","YSR")
plotPartnerDeletionsHist(transcripts_sp1=transcripts$Oki, transcripts_sp1= bigChr=big_chr)
```

```{r gene_oki_kum_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
# Given a pair of chromosome-scale orthopairs, 
plotValidationSpDeletions <- function(orthoPair, orthoFlag="deletion_species_specific", flag=NULL, facet.by="seqnames~Arm", bigChr=NULL, binWidth=100000) {
  orthoPair$midpoint <- grMidpoint(orthoPair)
  df <- as.data.frame(orthoPair)
  df <- subset(df, df$seqnames %in% bigChr)
  df <- df[which(unlist(lapply(df$orthoFlagDF, function(x) x[[orthoFlag]])) == flag),]
...
big_chr = c("chr1","chr2","PAR","XSR","YSR")
plotValidationSpDeletions(orthoPairs$Oki_Osa, orthoFlag="deletion_species_specific", flag="Kum", facet.by="~Arm", bigChr=big_chr, binWidth=100000)
```
