...
Species- and clade-specific duplications across genome
------------------------------------------------------
...
duplications$Oki_Kum <- collect_clade_expansions(orthopairs=orthoPairs, sp1="Oki", sp2="Kum", bigChr=big_chr, longShort=longShort$Oki)
big_chr <- c("Chr1","Chr2","PAR","XSR","YSR")
duplications$Osa_Aom <- collect_clade_expansions(orthopairs=orthoPairs, sp1="Osa", sp2="Aom", bigChr=big_chr, longShort=longShort$Osa)
duplications$Bar_Nor <- collect_clade_expansions(orthopairs=orthoPairs, sp1="Bar", sp2="Nor", bigChr=big_chr, longShort=longShort$Bar)
```
...
Now we have data frames that count up the number of clade- and species-specific genomes. Where do duplications fall in the genome?

```{r gene_duplications_pos_oki_1, fig.height=4, fig.width=6, dev=c("svg", "png")}
# Make sure factor levels are in alphabetical order. Otherwise the faceting looks ugly.
duplications$Oki_Kum  <- lapply(duplications$Oki_Kum, function(sp){
...
ggplot(duplications$Oki_Kum$Oki) +
  ...
  geom_histogram(binwidth=100000) +
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Species-specific gene family expansions: Oki')
...
ggplot(duplications$Oki_Kum$Kum) +
  ...
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Species-specific gene family expansions: Kum')
...
calcExpansionStats <- function(duplication_pair, pair_name) {
  df <- do.call(rbind, duplication_pair) |> mutate(bin=cut_width(midpoint, 100000)) |> dplyr::group_by(seqnames, Arm, bin) |> dplyr::summarise(count=dplyr::n())
  # Show some numbers...
  anova = aov(df$count ~ df$seqnames + df$Arm) |> summary() 
  tukey = TukeyHSD(aov(df$count ~ df$seqnames + df$Arm), conf.level=0.95)
  return(SimpleList(anova=anova, tukey=tukey))
}
stats <- calcExpansionStats(duplications$Oki_Kum, pair_name = "Oki-Kum")
stats$anova
stats$tukey
```
...
duplications$Osa_Aom  <- lapply(duplications$Osa_Aom, function(sp){
  sp$seqnames <- droplevels(factor(sp$seqnames, levels=sort(as.character(unique(sp$seqnames)))))
  sp
})
...
ggplot(duplications$Osa_Aom$Osa_Aom) +
  ...
  geom_histogram(binwidth=100000) +
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Gene family expansions: Osa-Aom')

ggplot(duplications$Osa_Aom$Osa) +
  aes(x=midpoint) +
  geom_histogram(binwidth=100000) +
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Species-specific gene family expansions: Osa')

ggplot(duplications$Osa_Aom$Aom) +
  aes(x=midpoint) +
  geom_histogram(binwidth=100000) +
  facet_wrap(seqnames~Arm, scales = "free_x", ncol=2) +
  ggtitle('Species-specific gene family expansions: Aom')
...
duplications$Bar_Nor  <- lapply(duplications$Bar_Nor, function(sp){
  sp$seqnames <- droplevels(factor(sp$seqnames, levels=sort(as.character(unique(sp$seqnames)))))
  sp
})
