...
```{r guess_operons}
operons <- lapply(annots, \(a) {a |> genes() |> OikScrambling:::calcOperons(window = 400) }) |> SimpleList() |> suppressWarnings()
...
prepareFreqPlot_coding <- function(gb, annot, rep = NULL, cne = NULL, tfbs = NULL, operons = NULL, win = 1000, direction = "left") {
  l <- list()
  ...
  #l$cds    <- feature_coverage(gb,     cds(annot),           win = win, lab = "CDS",    direction = direction)
...
prepareFreqPlot_noncoding <- function(gb, annot, rep, cne = NULL, tfbs = NULL, operons = NULL, win = 1000, direction = "left") {
  l <- list()
  # Note that get_bps(direction="mid") is probably not what you think.
  if(direction == "mid") {
    gb <- resize(gb, 1, "center")
...
computePlotData <- function(target, query, perpareFreqPlot, reps = NULL, CE=NULL, tfbs=NULL, operons = NULL, direction = "left") {
  ...
  prepareFreqPlot_default <- \(x) perpareFreqPlot(x, annots[[target]], reps[[targetGenome]], CE, tfbs, operons[[target]], direction = direction)
...
hmList_to_plot_noncoding <- computePlotData("Oki", "Osa", prepareFreqPlot_noncoding, reps = reps, CE = ces$ce__50__48$Oki_Osa, tfbs = NULL)
hmList_to_plot_coding    <- computePlotData("Oki", "Osa", prepareFreqPlot_coding, reps = reps, CE = ces$ce__50__48$Oki_Osa, tfbs = NULL)
...
hmList_to_plot.centered <- computePlotData("Oki", "Osa", ces$ce__50__48$Oki_Osa, tfbs, operons, direction = "mid")
...
(p_boundary <- rbind(
  hmList_to_plot.centered$aln_nonCoa  |> prepareGGFreqPlot("01_isolated_alns"),
  hmList_to_plot.centered$unmap       |> prepareGGFreqPlot("02_unmapped"),
  hmList_to_plot.centered$syn_block   |> prepareGGFreqPlot("05_colinear_reg"),
  hmList_to_plot.centered$bri         |> prepareGGFreqPlot("04_bridge"),
  hmList_to_plot.centered$aln_coa     |> prepareGGFreqPlot("03_colinear_alns")
...
feature_coverage_on_operons <- {
  # Hide the pre-computed object in the function's environment
  operon_boundaries <- c(
    operons$Oki |> resize(1, fix = "start"),
    operons$Oki |> resize(1, fix = "end")
  ) |> sort(i = T)
  function(feat, desc)
    feature_coverage(gene_boundaries, feat, window = 1000, lab = "", dir = "left") |> Heatmap_to_freq_tibble(desc)
}

rbind(
  feature_coverage_on_operons(gbs$Oki_Osa[gbs$Oki_Osa$nonCoa], "01_isolated"),
  feature_coverage_on_operons(unal$Oki_Osa[unal$Oki_Osa$nonCoa], "02_unaligned"),
  feature_coverage_on_operons(gbs$Oki_Osa[!gbs$Oki_Osa$nonCoa], "03_colinear_aln"),
  feature_coverage_on_operons(bri$Oki_Osa, "04_bridge"),
  feature_coverage_on_operons(coa$Oki_Osa[!coa$Oki_Osa$nonCoa], "05_colinear_reg")
) |> ggplot() + aes(pos, freq, col = desc) + geom_line() + ggtitle("operons")
```
...
feature_coverage_on_genes <- {
  # Hide the pre-computed object in the function's environment
  gene_boundaries <- c(
    annots$Oki |> genes() |> resize(1, fix = "start"),
    annots$Oki |> genes() |> resize(1, fix = "end")
  ) |> sort(i = T)
  function(feat, desc)
    feature_coverage(gene_boundaries, feat, window = 1000, lab = "", dir = "left") |> Heatmap_to_freq_tibble(desc)
}
...
rbind(
  ...
  feature_coverage_on_genes(unal$Oki_Osa[unal$Oki_Osa$nonCoa], "02_unaligned"),
...
) |> ggplot() + aes(pos, freq, col = desc) + geom_line() + ggtitle("genes")

rbind(
  feature_coverage_on_genes(gbs$Oki_Osa[gbs$Oki_Osa$nonCoa] |> resize(1), "01_isolated"),
  feature_coverage_on_genes(unal$Oki_Osa[unal$Oki_Osa$nonCoa]|> resize(1), "02_unaligned"),
  feature_coverage_on_genes(gbs$Oki_Osa[!gbs$Oki_Osa$nonCoa]|> resize(1), "03_colinear_aln"),
  feature_coverage_on_genes(bri$Oki_Osa|> resize(1), "04_bridge"),
  feature_coverage_on_genes(coa$Oki_Osa[!coa$Oki_Osa$nonCoa]|> resize(1), "05_colinear_reg")
) |> ggplot() + aes(pos, freq, col = desc) + geom_line() + ggtitle("genes")
```
