...
```{r}
op <- sort(genes(annots$Oki), ignore.strand=T)
op <- trim(op + 200) |> suppressWarnings()
# https://stackoverflow.com/questions/21421047
# op$idx <- data.table::rleid(as.character(strand(op)))
op$idx <- Rle(rep(seq_along(runValue(strand(op))), runLength(strand(op))))
opL <- split(op, op$idx)
operons <- unlist(endoapply(opL, reduce))
op <- op - 200
operons <- operons - 200
operons <- operons[!operons %in% op]
```

## Accessory functions

```{r genomic_features_byType_accessory_Functions}
prepareFreqPlot <- function(gb, annot, rep, cne = NULL, tfbs = NULL, win = 1000) {
  l <- list()
...
computePlotData <- function(target, query, CE=NULL, tfbs=NULL) {
  ...
  hmList <- SimpleList()
  hmList$aln_nonCoa <- prepareFreqPlot(current_GB_aln_nonCoa, annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList$aln_coa    <- prepareFreqPlot(current_GB_aln_coa,    annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList$syn_block  <- prepareFreqPlot(current_GB_syn_block,  annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList$unmap      <- prepareFreqPlot(current_GB_unmap,      annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList$bri    <- prepareFreqPlot(current_GB_bri,    annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList
}

hmList_to_plot <- computePlotData("Oki", "Osa", ces$ce__50__48$Oki_Osa, tfbs)
