...
test <- FALSE
# If target gene not in orthogroup table, it is species-specific and therefore FALSE
if(target_gene %in% unlist(pho[[target_sp]]) && query_gene %in% unlist(pho[[query_sp]]) && !is.na(target_gene) && !is.na(query_gene) ) {
...
compare_operons(pho=pho, scrambled_operon_example, operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location="ends")
# But "any" is true.
compare_operons(pho=pho, scrambled_operon_example, operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location = "any")
```
