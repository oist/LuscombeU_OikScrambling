...
computePlotData <- function(target, query, CE=NULL, tfbs=NULL) {
  ...
  # We do not use unal, because it overlaps with unalMap
  current_GB_unmap          <- unmap[[pair]]
  current_GB_unalMap        <- unalMap[[pair]]
  ...
  hmList$unalMap    <- prepareFreqPlot(current_GB_unalMap,    annots[[target]], reps[[targetGenome]], CE, tfbs)
  hmList
}
...
(p_boundary <- rbind(
  ...
  hmList_to_plot$unalMap     |> prepareGGFreqPlot("04_unal_syntenic"),
...
# hmListUnalMap_c <- prepareFreqPlot(current_GB_unalMap_c, annots$Oki, reps$Oki, currentCNE)
# 
# p_center <- rbind(
