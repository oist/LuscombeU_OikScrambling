...
rbind(
  operons$Oki |> as.data.frame() |> mutate(species="Okinawa"),
  operons$Osa |> as.data.frame() |> mutate(species="Osaka"),
  operons$Bar |> as.data.frame() |> mutate(species="Barcelona")
) |> ggplot() +
  aes(y=factor(n)) +
  geom_bar() +
...
rbind(oki_feat_bp_ovl, osa_feat_bp_ovl, bar_feat_bp_ovl) |>
  ggplot() +
  aes(pct, type, col = type, pch = species, size = 2) +
  geom_point() +
  scale_y_discrete("") +
  scale_x_continuous("Percentage of features overlaping a breakpoint region") +
  theme_bw()
```
