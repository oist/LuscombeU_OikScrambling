...
appears to be conserved in OKI and OSA.  In OKI:
...
```{r ganot_operon}
transcripts$Oki |> subsetByOverlaps(GRanges("PAR:15956430-15967745"))
transcripts$Oki |> subsetByOverlaps(GRanges("PAR:15956430-15967745")) |> cleanGaps() |> width()
```
...
) |> ggplot() + aes(pos, freq, col = desc) + geom_line() + ggtitle("genes_internal_to_operons")
...
rbind(
  ...
  feature_coverage_on_genes_outside(unal$Oki_Osa[unal$Oki_Osa$nonCoa], "02_unaligned"),
...
) |> ggplot() + aes(pos, freq, col = desc) + geom_line() + ggtitle("genes_outside_operons")
```
...
operons$Oki$n |> table()
subsetByOverlaps(operons$Oki, unal$Oki_Osa[unal$Oki_Osa$nonCoa])$n |> table()
```
