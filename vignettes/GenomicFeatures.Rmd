...
prepareFreqPlot <- function(gb, annot, rep, cne = NULL, win = 1000) {
  l <- list()
...
computePlotData <- function(target, query, CE=NULL) {
  ...
  hmList <- SimpleList()
  hmList$aln_nonCoa <- prepareFreqPlot(current_GB_aln_nonCoa, annots[[target]], reps[[target]], CE)
  hmList$aln_coa    <- prepareFreqPlot(current_GB_aln_coa,    annots[[target]], reps[[target]], CE)
  hmList$syn_block  <- prepareFreqPlot(current_GB_syn_block,  annots[[target]], reps[[target]], CE)
  hmList$unmap      <- prepareFreqPlot(current_GB_unmap,      annots[[target]], reps[[target]], CE)
  hmList$unalMap    <- prepareFreqPlot(current_GB_unalMap,    annots[[target]], reps[[target]], CE)
  hmList
}

hmList_to_plot <- computePlotData("Oki", "Osa", ces$ce__50__48$Oki_Osa)
