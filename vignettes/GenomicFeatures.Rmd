...
This section uses two key functions, `single_gene_equivalence` and `operon_equivalence`.
* `single_gene_equivalence`: Input is an `orthoPair` `GBreaks` object, and two gene identifiers (one for the query, one for the target). It checks whether the two gene identifiers are equivalent according to the `criterion` argument. There are two options for equivalence, `one_to_one` (strict) or `same_hierarchical_orthogroup` (slightly more permissive), or `same_orthogroup` (extremely permissive).
...
single_gene_equivalence <- function(pho, target_gene, target_sp, query_sp, criterion="one_to_one"){
  equivalence <- FALSE
...
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g16808")
(pho[pho$Oki == "g10565",]) # Non-PHO gene
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g10565")
(pho[pho$Oki == "g9611",]) # Non-single-copy gene
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g9611")
# Hierarchical groups
(pho[pho$Oki == "g16808",]) # Single-copy gene
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g16808", criterion="same_hierarchical_orthogroup")
(pho[pho$Oki == "g10565",]) # Non-PHO gene
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g10565", criterion="same_hierarchical_orthogroup")
(pho[pho$Oki == "g9611",]) # Non-single-copy gene
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g9611", criterion="same_hierarchical_orthogroup")
# Orthogroups
(pho[pho$Oki == "g10223",]) # Big orthogroup with many HOGs
(pho[pho$OG  == "OG0000000",])
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g10223", criterion="same_hierarchical_orthogroup")
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g10223", criterion="same_orthogroup")
...
single_gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g7782", criterion="same_orthogroup")
# ...but exists in Barcelona.
single_gene_equivalence(pho, target_sp="Oki", query_sp="Bar", target_gene="g7782", criterion="same_orthogroup")
```
...
operon_equivalence <- function(pho, operons_sp1, operons_sp2, target_sp=NULL, query_sp=NULL, single_gene_criterion="one_to_one", min_size=1, operon_criterion='exact' ){
  ...
  checks_sp1 <- lapply(operons_sp1$gene_id, function(operon_gene_list) {
    equivalent <- FALSE
    gene_checks <- lapply(operon_gene_list, function(target_gene){
      single_gene_equivalence(pho=pho, target_gene=target_gene, criterion = single_gene_criterion, target_sp=target_sp, query_sp=query_sp)
    }) |> unlist()
