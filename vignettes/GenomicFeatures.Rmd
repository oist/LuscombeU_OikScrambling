...
### Confirming the validity of operon intergenic distances
One of our key observations is that operons are not conserved. Is that because of our permissive cutoff, allowing many false-positive operons to be discovered?

#### Creating operons of different sizes
...
#### Preparing orthologue set for comparisons
...
gene_equivalence <- function(pho, target_gene, target_sp, query_sp, criterion="one_to_one"){
  equivalence <- FALSE
  pho_target_sp <- pho[[target_sp]]
  pho_query_sp  <- pho[[query_sp]]
  # If target gene not in orthogroup table, it is species-specific and therefore FALSE
  if(target_gene %in% unlist(pho_target_sp)) {
    # This is the fast way
    d_t <- as.data.frame(pho_target_sp)
    matching_target_index <- d_t$group[which(d_t$value==target_gene)]

    target_hog <- pho_target_sp[matching_target_index] |> unlist()
    query_hog  <- pho_target_sp[matching_target_index]  |> unlist()
    if(criterion=="one_to_one"){
      ...
      if(!all(is.na(target_hog)) && length(target_hog)>=1) {
        ...
        query_sp_in_target_og <- pho_query_sp[pho$OG == target_og]
...
#for(i in 1:100) { gene_equivalence(pho, target_sp="Oki", query_sp="Osa", target_gene="g16808") } |> system.time()
...
gene_equivalents <- function(pho, target_gene, target_sp, query_sp, criterion="one_to_one"){
  ...
  if(target_gene %in% unlist(pho[[target_sp]])) {
    ...
    matching_target_index <- d_t$group[which(d_t$value==target_gene)]
    target_hog <- pho_target_sp[matching_target_index] |> unlist()
    query_hog  <- pho_query_sp[matching_target_index]   |> unlist()
    if(criterion=="one_to_one"){
      if(!all(is.na(target_hog)) && !all(is.na(query_hog)) && length(target_hog)==1 && length(query_hog)==1){
      ...
      if(!all(is.na(target_hog)) && length(target_hog)>=1) {
        ...
        any_query_sp_in_target_og <- any(unlist(lapply(query_sp_in_target_og, function(x) !is.na(x) )))
        if(any_query_sp_in_target_og){
          equivalents <- query_sp_in_target_og |> unlist() |> na.omit() |> as.character()
        }
      }
    }
  }
...
# (pho[pho$Oki == "g10565",]) # Non-PHO gene
gene_equivalents(pho, target_sp="Oki", query_sp="Osa", target_gene="g10565")
# (pho[pho$Oki == "g9611",]) # Non-single-copy gene
gene_equivalents(pho, target_sp="Oki", query_sp="Osa", target_gene="g9611")
# Hierarchical groups
# (pho[pho$Oki == "g16808",]) # Single-copy gene
...
gene_equivalence_test <- function(pho, target_gene, target_sp, query_gene, query_sp, criterion="one_to_one"){
  ...
  pho_target_sp <- pho[[target_sp]]
  pho_query_sp  <- pho[[query_sp]]
  pho_target_sp_unl <- unlist(pho[[target_sp]])
  pho_query_sp_unl  <- unlist(pho[[query_sp]])
  
  # If target gene not in orthogroup table, it is species-specific and therefore FALSE
  if(target_gene %in% pho_target_sp_unl && query_gene %in% pho_query_sp_unl && !is.na(target_gene) && !is.na(query_gene) ) {
    d_t <- as.data.frame(pho[[target_sp]])
    d_q <- as.data.frame(pho[[query_sp]])
    matching_target_index <- d_t$group[which(d_t$value==target_gene)]
    matching_query_index  <- d_q$group[which(d_q$value==query_gene)]

    g_t <- pho_target_sp[matching_target_index] |> unlist()
    g_q <- pho_query_sp[matching_query_index]   |> unlist()
    if(criterion=="one_to_one") {
...
# system.time(for(i in 1:100) { gene_equivalence_test(pho=pho, target_gene = "g16808", target_sp = "Oki", query_gene="g12947", query_sp="Osa", criterion = "one_to_one")  } )
# (pho[pho$Oki == "g16808",]) # Single-copy gene
...
operon_of_gene <- function(id, op) {
  ...
  # faster method
  d <- as.data.frame(op$gene_id)
  matching_operon <- d[d$value==id,]$group
  op[matching_operon]
}
# for performance optimization
# system.time(for(i in 1:1000) { operon_of_gene("g57", operons$Oki) })
operon_of_gene("g57", operons$Oki)
operon_of_gene("g500000", operons$Oki)

which_operon <- function(id, op) {
  result <- which(lapply(op$gene_id, function(o) id %in% o) |> unlist()) |> unname()
  result
}
...
#### Aside: The easiest-possible case of operon conservation
The easiest case of operon conservation is when all of the genes in an operon of one species are located in one operon of the same size in species 2, and all of the genes involved are single-copy orthologues.

When genes are not single-copy orthologues, it is not certain whether gene A in species 1 is A1 or A2 in species 2. When collinear single-copy orthologues between both species participate in operons of different length, it could be due to the insertion/deletion of one or more genes in the operon in one species. Or, it could be cause by any other artifact of the method used to define operons here.

Anyway, how common is the easiest-possible case of operon conservation? We can assess the number of single-copy-gene-operons as a way to frame our thinking about these operon annotations.

```{r easy_operon_criterion}
# A list version of pho is easier to look at here.
pho2 <- read.delim(system.file("extdata/OrthoFinder/N19.tsv", package="BreakpointsData"))
pho2 <- pho2[,!apply(pho2, 2, function(d) all(is.na(d)))]
colnames(pho2) <- c('HOG', 'OG', 'GTPC', 'Aom', 'Bar', 'Kum', 'Oki', 'Osa', 'Nor')
of_to_list <- function(of_ids, gene_id_separator='\\.', gene_id_field=2){
  of_list <- strsplit(of_ids, ', ')
  lapply(of_list, function(hog) {
    if(length(hog)==0){
...
pho2$Aom <- of_to_list(pho2$Aom, gene_id_field = 2)
pho2$Bar <- of_to_list(pho2$Bar, gene_id_field = 2)
pho2$Kum <- of_to_list(pho2$Kum, gene_id_field = 2)
pho2$Oki <- of_to_list(pho2$Oki, gene_id_field = 3)
pho2$Osa <- of_to_list(pho2$Osa, gene_id_field = 3)
pho2$Nor <- of_to_list(pho2$Nor, gene_id_field = 2)

pho2$Oki_SC <- unlist(lapply(pho2$Oki, function(x) length(na.omit(x))==1))
pho2$Osa_SC <- unlist(lapply(pho2$Osa, function(x) length(na.omit(x))==1))
pho2$Bar_SC <- unlist(lapply(pho2$Bar, function(x) length(na.omit(x))==1))

# All genes
sapply(c("Oki", "Osa", "Bar"), function(s) length(genes[[s]]))

# All operons
sapply(c("Oki", "Osa", "Bar"), function(s) length(operons[[s]]))

# All genes in operons
sapply(c("Oki", "Osa", "Bar"), function(s) sum(operons[[s]]$n)  )

# All genes represented in any orthogroup
sapply(c("Oki", "Osa", "Bar"), function(s) sum(unlist(lapply(pho[[s]], function(x) length(na.omit(x))))) )

# Subset to single-copy orthologues only
pho2_sco <- pho2[apply(pho2[,10:12], 1, all),]
sapply(c("Oki", "Osa", "Bar"), function(s) sum(unlist(lapply(pho2_sco[[s]], function(x) length(na.omit(x))))) )

pho2_sco$Oki_op <- lapply( pho2_sco$Oki, function(g) all(g %in% unlist(operons$Oki$gene_id))) |> unlist()
pho2_sco$Osa_op <- lapply( pho2_sco$Osa, function(g) all(g %in% unlist(operons$Osa$gene_id))) |> unlist()
pho2_sco$Bar_op <- lapply( pho2_sco$Bar, function(g) all(g %in% unlist(operons$Bar$gene_id))) |> unlist()

# Single-copy orthologues in ANY operon
sapply(c("Oki", "Osa", "Bar"), function(s) length(which(pho2_sco[[paste0(s, "_op")]])) )

named_operons <- SimpleList()
named_operons$Oki <- operons$Oki
named_operons$Osa <- operons$Osa
named_operons$Bar <- operons$Bar
names(named_operons$Oki) <- paste0("o", 1:length(named_operons$Oki))
names(named_operons$Osa) <- paste0("o", 1:length(named_operons$Osa))
names(named_operons$Bar) <- paste0("o", 1:length(named_operons$Bar))

pho2_sco$Oki_op_names <- lapply( pho2_sco$Oki, function(g) names(operon_of_gene(g, op = named_operons$Oki)))
pho2_sco$Osa_op_names <- lapply( pho2_sco$Osa, function(g) names(operon_of_gene(g, op = named_operons$Osa)))
pho2_sco$Bar_op_names <- lapply( pho2_sco$Bar, function(g) names(operon_of_gene(g, op = named_operons$Bar)))

pho2_sco$Oki_op_length <- NA
pho2_sco$Oki_op_length[lengths(pho2_sco$Oki_op_names) > 0] <- named_operons$Oki[unlist(pho2_sco[lengths(pho2_sco$Oki_op_names)>0,]$Oki_op_names),]$n
pho2_sco$Osa_op_length <- NA
pho2_sco$Osa_op_length[lengths(pho2_sco$Osa_op_names) > 0] <- named_operons$Osa[unlist(pho2_sco[lengths(pho2_sco$Osa_op_names)>0,]$Osa_op_names),]$n
pho2_sco$Bar_op_length <- NA
pho2_sco$Bar_op_length[lengths(pho2_sco$Bar_op_names) > 0] <- named_operons$Bar[unlist(pho2_sco[lengths(pho2_sco$Bar_op_names)>0,]$Bar_op_names),]$n

# Single-copy orthologues in an operon
pho2_sco_op <- pho2_sco[apply(pho2_sco[,13:15], 1, all),]

pho2_sco_op$Oki_op_gr <- lapply(pho2_sco_op$Oki_op_names, function(x) named_operons$Oki[x]) |> as("GRangesList") |> unlist()
pho2_sco_op$Osa_op_gr <- lapply(pho2_sco_op$Osa_op_names, function(x) named_operons$Osa[x]) |> as("GRangesList") |> unlist()
pho2_sco_op$Bar_op_gr <- lapply(pho2_sco_op$Bar_op_names, function(x) named_operons$Bar[x]) |> as("GRangesList") |> unlist()

pho2_sco_op <- pho2_sco_op |> DataFrame()

pho2_sco_op$Oki_op_gr_geneid <- pho2_sco_op$Oki_op_gr$gene_id
pho2_sco_op$Osa_op_gr_geneid <- pho2_sco_op$Osa_op_gr$gene_id
pho2_sco_op$Bar_op_gr_geneid <- pho2_sco_op$Bar_op_gr$gene_id

pho2_sco_op |> nrow()

# Now, genes that are only in operons of the same length
pho2_sco_op_samelen <- subset(pho2_sco_op, Oki_op_length == Osa_op_length & Oki_op_length == Bar_op_length)
# How many genes and operons are left?
nrow(pho2_sco_op_samelen)
sapply(c("Oki", "Osa", "Bar"), function(s) sum(pho2_sco_op_samelen[[paste0(s, "_op_length")]])  )


pho2_sco_op_samelen$Oki_Osa_op_equivalence <- lapply(1:nrow(pho2_sco_op_samelen), function(o) {
  equivalence <- FALSE
  ge_tests <- lapply(1:length(pho2_sco_op_samelen$Oki_op_gr_geneid[o]), function(g){
    t_g <- pho2_sco_op_samelen$Oki_op_gr_geneid[[o]][[g]] |> unlist()
    q_g <- pho2_sco_op_samelen$Osa_op_gr_geneid[[o]][[g]] |> unlist()
    ge <- gene_equivalence_test(pho=pho, target_gene=t_g, query_gene = q_g, target_sp = "Oki", query_sp = "Osa")
  }) |> unlist()
...
pho2_sco_op_samelen$Oki_Bar_op_equivalence <- lapply(1:nrow(pho2_sco_op_samelen), function(o) {
  equivalence <- FALSE
  ge_tests <- lapply(1:length(pho2_sco_op_samelen$Oki_op_gr_geneid[o]), function(g){
    t_g <- pho2_sco_op_samelen$Oki_op_gr_geneid[[o]][[g]] |> unlist()
    q_g <- pho2_sco_op_samelen$Bar_op_gr_geneid[[o]][[g]] |> unlist()
    ge <- gene_equivalence_test(pho=pho, target_gene=t_g, query_gene = q_g, target_sp = "Oki", query_sp = "Bar")
  }) |> unlist()
...
pho2_sco_op_samelen$Osa_Bar_op_equivalence <- lapply(1:nrow(pho2_sco_op_samelen), function(o) {
  equivalence <- FALSE
  ge_tests <- lapply(1:length(pho2_sco_op_samelen$Osa_op_gr_geneid[o]), function(g){
    t_g <- pho2_sco_op_samelen$Osa_op_gr_geneid[[o]][[g]] |> unlist()
    q_g <- pho2_sco_op_samelen$Bar_op_gr_geneid[[o]][[g]] |> unlist()
    ge <- gene_equivalence_test(pho=pho, target_gene=t_g, query_gene = q_g, target_sp = "Osa", query_sp = "Bar")
  }) |> unlist()
...
# How many operons are EXACTLY equivalent?
pho2_sco_op_samelen$Oki_Osa_op_equivalence |> table()
pho2_sco_op_samelen$Oki_Bar_op_equivalence |> table()
pho2_sco_op_samelen$Osa_Bar_op_equivalence |> table()

pho2_sco_op_samelen_sameoperon <- subset(pho2_sco_op_samelen, Oki_Osa_op_equivalence==TRUE & Osa_Bar_op_equivalence==TRUE)
nrow(pho2_sco_op_samelen_sameoperon)
sapply(c("Oki", "Osa", "Bar"), function(s) sum(pho2_sco_op_samelen[[paste0(s, "_op_length")]])  )


# Account for genes in their sets...
oki_gene_accounting <- list(
  All_genes=genes$Oki$gene_id,
  Genes_in_operons=unlist(operons$Oki$gene_id),
  SCOs=unlist(pho2_sco$Oki),
  SCOs_in_operons=unlist(pho2_sco_op$Oki),
  #SCOs_in_operon_conserved_size=unlist(pho2_sco_op_samelen$Oki),
  SCO_operon=unlist(pho2_sco_op_samelen_sameoperon$Oki)
)

lapply(oki_gene_accounting, length) |> as.data.frame() |> t() |> as.data.frame() |> tibble::rownames_to_column() |> dplyr::mutate(gene_set=rowname, count=V1) |> ggplot() + aes(x=factor(gene_set, levels=gene_set), y=count) + geom_bar(stat='identity') + xlab("Gene set") + ggtitle("Okinawa: Number of genes in operon category") 
# If such a package were to exist we could do:
# oki_gene_accounting |> eulerr::euler() |> plot(quantities=TRUE)

osa_gene_accounting <- list(
  All_genes=genes$Osa$gene_id,
  Genes_in_operons=unlist(operons$Osa$gene_id),
  SCOs=unlist(pho2_sco$Osa),
  SCOs_in_operons=unlist(pho2_sco_op$Osa),
  #SCOs_in_operon_conserved_size=unlist(pho2_sco_op_samelen$Osa),
  SCO_operon=unlist(pho2_sco_op_samelen_sameoperon$Osa)
)

lapply(osa_gene_accounting, length) |> as.data.frame() |> t() |> as.data.frame() |> tibble::rownames_to_column() |> dplyr::mutate(gene_set=rowname, count=V1) |> ggplot() + aes(x=factor(gene_set, levels=gene_set), y=count) + geom_bar(stat='identity') + xlab("Gene set") + ggtitle("Osaka: Number of genes in operon category") 


bar_gene_accounting <- list(
  All_genes=genes$Bar$gene_id,
  Genes_in_operons=unlist(operons$Bar$gene_id),
  SCOs=unlist(pho2_sco$Bar),
  SCOs_in_operons=unlist(pho2_sco_op$Bar),
  #SCOs_in_operon_conserved_size=unlist(pho2_sco_op_samelen$Bar),
  SCO_operon=unlist(pho2_sco_op_samelen_sameoperon$Bar)
)

lapply(bar_gene_accounting, length) |> as.data.frame() |> t() |> as.data.frame() |> tibble::rownames_to_column() |> dplyr::mutate(gene_set=rowname, count=V1) |> ggplot() + aes(x=factor(gene_set, levels=gene_set), y=count) + geom_bar(stat='identity') + xlab("Gene set") + ggtitle("Barcelona: Number of genes in operon category") 


```

#### Not-easy operon set comparison
Now to make a function that compares one operon to another operon given some gene matching and operon matching criteria.

This is a bit involved, and quite slow. Basically, the function has to check all the combinations of genes to check for weird cases. These are commented below.

```{r confirm_operon_size_5}
compare_operons <- function(op_sp1, op_sp2, operon_criterion="exact", pho, target_sp, query_sp, single_gene_criterion="one_to_one", drop_location="ends") {
  ...
  if(length(op_sp1)!=0 && length(op_sp2)!=0){
    ...
    if(operon_criterion=="exact") {
      ...
      if( ! abs(length(genes_sp1)-length(genes_sp2)) > 1  ) {
        # Because 1 gene can be dropped, and there are 2 different
        # strategies for dropping 1 gene, you have to check
        # whether every gene in sp1 has an equivalent gene in sp2.
        # So gene_checks becomes a matrix here rather than a list.
        gene_checks <- sapply(1:length(genes_sp1), function(i) {
          sapply(1:length(genes_sp2), function(j) {
            gene_equivalence_test(pho=pho, target_gene=genes_sp1[i], target_sp=target_sp, query_gene=genes_sp2[j], query_sp=query_sp, criterion=single_gene_criterion)
          })
        })
        all_sp1_checks <- apply(gene_checks, 2, any)
        
        if(all(all_sp1_checks)) {
          # Even in drop_1 mode, the two operons can still be exactly equivalent.
          equivalent <- TRUE
        } else if(drop_location=="ends") {
          # In "ends" mode, either the first or the last gene can be
          # FALSE in its gene check. Everything in the middle must be TRUE.
          drop_first <- all_sp1_checks[2:length(all_sp1_checks)]
          drop_last  <- all_sp1_checks[1:length(all_sp1_checks)-1]
          if(all(drop_first) | all(drop_last)) {
            equivalent <- TRUE
          }
        } else if(drop_location == "any") {
          # In "any" drop mode, any of the genes missing in one species
          # can be missing in the other species.
          # As long as the number of TRUE comparisons in all_sp1_checks
          # is 1 less than the number of genes in either species, then it's a match.
          if(length(which(all_sp1_checks)) >= max(c(length(genes_sp1), length(genes_sp2)))  - 1 ) {
            equivalent <- TRUE
          }
        }
      }
    } else if(operon_criterion=="drop_2") {
      # The two operons can be differ in length by at most 2 genes.
      if(! abs(length(genes_sp1)-length(genes_sp2)) > 2) {
        # Very similar to the above, otherwise.
        gene_checks <- sapply(1:length(genes_sp1), function(i) {
          sapply(1:length(genes_sp2), function(j) {
            gene_equivalence_test(pho=pho, target_gene=genes_sp1[i], target_sp=target_sp, query_gene=genes_sp2[j], query_sp=query_sp, criterion=single_gene_criterion)
          })
        ...
        if(all(all_sp1_checks)) {
          ...
          # drop_2 has more combinations that need considering.
          # In "ends" mode, either the first and last can be FALSE,
          # or the first 2 can be FALSE, or the last 2 can be FALSE.
          drop_first <- tail(all_sp1_checks, n=-2)
          drop_last  <- head(all_sp1_checks, n=-2)
          drop_edges <- tail(head(all_sp1_checks, n=-1), n=-1)
          
          if(all(drop_first) | all(drop_last) | all(drop_edges)) {
            equivalent <- TRUE
          }
        } else if(drop_location == "any") {
          # In "any" drop mode, any of the genes missing in one species
...
#system.time( for(i in 1:1000) { operon_of_gene("g1814", operons$Oki) }  )
#stop()
# A probable example of an operon with a single missing gene in Osaka.
# This is a case where "drop_1" is helpful, as the two operons are 
# identical except for one gene on the end.
operon_of_gene("g1814", operons$Oki)

# Check the orthology of every gene in the operon with the following.
# Names are genes in target, result is gene in query.
sapply(unlist(operon_of_gene("g1814", operons$Oki)$gene_id), function(gene) gene_equivalents(pho, gene, target_sp="Oki", query_sp="Osa"))
operon_of_gene("g1120", operons$Osa)
sapply(unlist(operon_of_gene("g1120", operons$Osa)$gene_id), function(gene) gene_equivalents(pho, gene, target_sp="Osa", query_sp="Oki"))
...
compare_operons(pho=pho, operon_of_gene("g1814", operons$Oki), operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location="ends")
compare_operons(pho=pho, operon_of_gene("g1814", operons$Oki), operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location = "any")

# Make a fake operon where the gene order is wrong, to test the "any" parameter.
set.seed(1992)
scrambled_operon_example_oki <- operon_of_gene("g1814", operons$Oki)
scrambled_operon_example_oki$gene_id[[1]] <- sample(scrambled_operon_example_oki$gene_id[[1]], 4)
# 14 = NA, 15=20, 16=19, 17=18
# So "ends" is wrong, here...
compare_operons(pho=pho, scrambled_operon_example_oki, operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location="ends")
# But "any" is true.
compare_operons(pho=pho, scrambled_operon_example_oki, operon_of_gene("g1120", operons$Osa), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_1", drop_location = "any")

# And finally, drop_2 mode. 
# Note that "ends" can drop either the first 2, the last 2, or one from both ends and be TRUE.
scrambled_operon_example_oki <- operon_of_gene("g1814", operons$Oki)
scrambled_operon_example_osa <- operon_of_gene("g1120", operons$Osa)
# Add genes so that 2 are missing. 1813 and 1814 are NA in this operon in Osaka.
# Gene equivalencies are FALSE, FALSE, TRUE, TRUE, TRUE, and operon equivalency is TRUE
scrambled_operon_example_oki$gene_id[[1]] <- c("g1813", "g1814", "g1815", "g1816", "g1817")
compare_operons(pho=pho, scrambled_operon_example_oki, scrambled_operon_example_osa, target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_2", drop_location = "ends")
# Gene equivalencies are FALSE, TRUE, TRUE, TRUE, FALSE, and operon equivalency is TRUE
scrambled_operon_example_oki$gene_id[[1]] <- c("g1814", "g1815", "g1816", "g1817", "g1818")
compare_operons(pho=pho, scrambled_operon_example_oki, scrambled_operon_example_osa, target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_2", drop_location = "ends")
# Gene equivalencies are FALSE, TRUE, FALSE TRUE, TRUE, and operon equivalency is TRUE
scrambled_operon_example_oki$gene_id[[1]] <- c("g1814", "g1815", "g1818", "g1816", "g1817")
compare_operons(pho=pho, scrambled_operon_example_oki, scrambled_operon_example_osa, target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_2", drop_location = "ends")
compare_operons(pho=pho, scrambled_operon_example_oki, scrambled_operon_example_osa, target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "drop_2", drop_location = "any")
```

Finally, compare all of the operons in one species to the operons of another species, using some arbitrary gene matching and operon matching combinations.

We'll be doing this all in parallel soon enough.

```{r confirm_operon_size_6}
# Helper function to compare one operon from a target to all of the operons in the query.
# I spent some time trying to get this down to the wire as best I could, but the function is still slow.
# It requires a lot of lapply()'ing.
compare_operons_grl_serial <- function(pho, target_operon, operon_grl, target_sp, query_sp, single_gene_criterion="one_to_one", operon_criterion="exact", drop_location="ends") {
  # Only need to do the comparison for ones that qualify. Don't bother, otherwise.
  matching_operons <- lapply(operon_grl, function(query_operon) {
    operon_match <- compare_operons(pho=pho, op_sp1=target_operon, op_sp2 = query_operon, target_sp=target_sp, query_sp=query_sp, single_gene_criterion = single_gene_criterion, operon_criterion = operon_criterion, drop_location = drop_location )
    operon_match
  })
  matching_operons
}
compare_operons_grl_serial(pho=pho, operons$Oki[6], split(operons$Osa, paste0("o", 1:length(operons$Osa))), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "exact") |> system.time()

compare_operons_grl_parallel <- function(pho, target_operon, operon_grl, target_sp, query_sp, single_gene_criterion="one_to_one", operon_criterion="exact", drop_location="ends") {
  # Only need to do the comparison for ones that qualify. Don't bother, otherwise.
  matching_operons <- BiocParallel::bplapply(operon_grl, function(query_operon) {
    operon_match <- compare_operons(pho=pho, op_sp1=target_operon, op_sp2 = query_operon, target_sp=target_sp, query_sp=query_sp, single_gene_criterion = single_gene_criterion, operon_criterion = operon_criterion, drop_location = drop_location )
    operon_match
  })
  matching_operons
}
compare_operons_grl_parallel(pho=pho, operons$Oki[6], split(operons$Osa, paste0("o", 1:length(operons$Osa))), target_sp="Oki", query_sp="Osa", single_gene_criterion = "one_to_one", operon_criterion = "exact") |> system.time()
...
operon_equivalence <- function(pho, operons_sp1, operons_sp2, target_sp=NULL, query_sp=NULL, single_gene_criterion="one_to_one", min_size=1, operon_criterion='exact', drop_location="ends"){
  # Make an object for storing the results of the comparison.
  result_df <- data.frame(
    sp1=target_sp, sp2=query_sp,
    single_gene_criterion=single_gene_criterion, operon_criterion=operon_criterion,
    drop_location=drop_location,
    n_operons_sp1=length(operons_sp1), n_operons_sp2=length(operons_sp2),
    min_size=min_size
  )
  
  # Remove operons that are too small and take notes
  operons_sp1 <- operons_sp1[operons_sp1$n>min_size]
  operons_sp2 <- operons_sp2[operons_sp2$n>min_size]
  result_df$n_operons_sp1_filter <- length(operons_sp1)
  result_df$n_operons_sp2_filter <- length(operons_sp2)
