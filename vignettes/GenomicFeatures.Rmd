...
```{r genomic_features_bpByWidth, cache=FALSE}
...
prepareFreqPlot <- function(gb, annot, win = 2.5e3) {
  l <- list()
  ...
  l$reps   <- feature_coverage(gb,     reps$Oki,             win = win, lab = "repeats",direction = "left")
  #reorder
...
prepareGGFreqPlot <- function(x, desc = NULL) {
  l <- lapply(names(x), \(name){
    ...
    what <- name
    tibble::tibble(val, freq = val / nrow(x[[name]]@image), what, pos = 1:length(val), desc)
  })
   do.call(rbind, l)
}

hmListWide    <- prepareFreqPlot(current_GB_wide,    annots$Oki)
hmListNarrow  <- prepareFreqPlot(current_GB_narrow,  annots$Oki)
hmListUnmap   <- prepareFreqPlot(current_GB_unmap,    annots$Oki)
hmListUnalMap <- prepareFreqPlot(current_GB_unalMap, annots$Oki)
...
As expected, both types of mapped regions are depleted for repeats.
Surprisingly, in both cases the depletion is relaxed after ~200 bp only.

```{r genomic_repeats_bpByWidth, cache=FALSE}
