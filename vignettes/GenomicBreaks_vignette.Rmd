...
For convenience, we provide a `genomes` object containing all the genomes.
...
```{r}
DF <- DataFrame(width = width(gbs$Oki_Osa), Arm = gbs$Oki_Osa$Arm, chr = seqnames(gbs$Oki_Osa))

gg_freq_poly(DF) +
  aes(col=Arm) +
  facet_wrap(~chr, scales = "free_y") +
  ggtitle("Distribution of alignment width before collapsing, split by arm.")
```
...
DF <- DataFrame(width = width(coa$Oki_Osa), Arm = coa$Oki_Osa$Arm, chr = seqnames(coa$Oki_Osa))

gg_freq_poly(DF) +
  aes(col=Arm) +
  facet_wrap(~chr, scales = "free_y") +
  ggtitle("Distribution of alignment width after collapsing, split by arm.") +
...
```{r re_coalesce}
DF <- DataFrame(width = width(coa2$Oki_Osa), Arm = coa2$Oki_Osa$Arm, chr = seqnames(coa2$Oki_Osa))

gg_freq_poly(DF) +
  aes(col=Arm) +
  facet_wrap(~chr, scales = "free_y") +
  ggtitle("Distribution of alignment width after double-collapsing, split by arm.") +
  geom_vline(xintercept = 500)
```
