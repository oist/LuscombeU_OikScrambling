...
See `vignette("RegionWidths"), package="OikSyntenies"`.
...
reps_in$total <- sapply(Oki_pairs, \(.) table(reps$Oki$Class)) 
reps_in$unal <- sapply(unal[Oki_pairs], \(gb) {
    ov <- subsetByOverlaps(reps$Oki, gb, ignore.strand = TRUE)
    table(ov$Class)
  })
...
reps_in$unmap <- sapply(unal[Oki_pairs], \(gb) {
    ov <- subsetByOverlaps(reps$Oki, gb, ignore.strand = TRUE)
    table(ov$Class)
  })
