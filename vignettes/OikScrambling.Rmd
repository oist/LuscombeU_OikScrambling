...
See `vignette("DistanceMeasures"), package="OikSyntenies"`.
...
# Make sure we match both strands
strand(reps) <- '*'

# Unique regions in the OKI genome that contain repeated elements.
reps <- reduce(reps)
...
(reps_gbs <- tibble::tibble(
    pairname   = Oki_pairs,
    total      = length(reps),
    match_unal = sapply(unal[Oki_pairs], \(gb) length(subsetByOverlaps(reps, gb))),
    match_al   = sapply(gbs[Oki_pairs], \(gb) length(subsetByOverlaps(reps, gb))),
...
(reps_coa <- tibble::tibble(
    pairname    = Oki_pairs,
    total       = length(reps),
    match_unmap = sapply(unal_c[Oki_pairs], \(gb) length(subsetByOverlaps(reps, gb))),
    match_map   = sapply(   coa[Oki_pairs], \(gb) length(subsetByOverlaps(reps, gb))),
