...
We used the functions `GenomicBreaks::scaf_align_plot()` and
`GenomicBreaks::align_scaf_plot_2()`to see the alignment of query
scaffolds/chromosomes onto a specific target scaffold/chromosome of interest.

Now that we have chromosome assemblies these functions are not much used anymore.
...
Pairwise chromosome plots like the one below are built in the _parallel plots_
vignette (`vignette("ParallelPlots", package = "OikScrambling")`).
...
Enrichement for motifs is being searched in
`vignette("Motif finding and analysis")`
