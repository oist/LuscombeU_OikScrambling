...
ggplot(as.data.frame(orthoPairs$Osa_Bar) %>% filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR'))) +
  aes(dNdS_GUIDANCE2, fill=Arm) +
  geom_boxplot() +
  coord_flip() +
  facet_wrap(~seqnames) +
...
ggplot(as.data.frame(orthoPairs$Bar_Osa) %>% filter(seqnames %in% c('Chr1', 'Chr2', 'PAR', 'XSR', 'YSR'))) +
  aes(dNdS_GUIDANCE2, fill=Arm) +
  geom_boxplot() +
  coord_flip() +
  facet_wrap(~seqnames) +
...
# Distribution of gene number in syntenic blocks
...
```{r exploring_dnds_4, fig.height=4, fig.width=6, dev=c("svg", "png")}
