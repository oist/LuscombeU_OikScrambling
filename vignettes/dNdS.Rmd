...
coalOrtho <- function(gb) {
  ...
  coal$dNdS <- BiocParallel::bplapply(geneNames, function(name) {
    if(length(name) == 1){
      dnds = dNdSFromOrthoPairs(gb, tx_name=name, column_name='dNdS_GUIDANCE2')
    } else {
      dnds = unlist(sapply(name, function(gene) {
        dNdSFromOrthoPairs(gb, tx_name=gene, column_name='dNdS_GUIDANCE2')
      }))
    }
    dnds
  })
