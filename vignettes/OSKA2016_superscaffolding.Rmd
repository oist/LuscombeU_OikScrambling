...
OSK421f <- load_genomic_breaks("~/FromSango/OSKA2016__OSK42-2f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, type = "match") |> coalesce_contigs()
...
OSK421f.cc <- strandNames(OSK421f.c, query = TRUE)
names(OSK421f.cc) <- paste("OSK421f", names(OSK421f.cc), sep = "_")
OSK421f.all  <- longestMatchesInTarget(OSK421f, min.width = 0, min.matches = 1) |> strandNames(q=T)
```
...
```{r OSK_42}
OSK421f.cc |> grepByContigName(name = "S10+")
assemble(OSK421f, "XSR", XSR) |> plotApairOfChrs()
assemble(OSK421f, "XSR", XSR) |> makeOxfordPlots()
```
