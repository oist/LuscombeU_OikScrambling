...
vignette: >
  ...
  %\VignetteIndexEntry{OSKA2016 superscaffolding}
...
library("BSgenome.Oidioi.Aniseed.OSKA2016")
```

# Prior information

Jan and me inspected by eye the dotplots of different genome alignments to
OSKA2016 and came to the conclusion that the following scaffolds belong to the
following chromosomes (or chromosome regions).
...
XSR.old <- c("S10+", "S9+", "S32+", "S84-", "S59+", "S8-", "S1+")
...
Custom functions that I use for exploring the data.
...
# As maf-convert was not run with -J for these genomes, `type="match"` is needed.
O97f <- load_genomic_breaks("~/FromSango/OSKA2016__O9-7f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, type = "match") |> coalesce_contigs()
genome(O97f$query) <- "O97f"
...
O35f <- load_genomic_breaks("~/FromSango/OSKA2016__O3-5f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, type = "match") |> coalesce_contigs()
genome(O35f) <- "O35f"

OSK42_2f <- load_genomic_breaks("~/FromSango/OSKA2016__OSK42-2f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, type = "match") |> coalesce_contigs()
genome(OSK42_2f$query) <- "OSK42_2f"
```

## Individuals captured in Aomori prefecture

```{r load_Aom}
AOM5.5f  <- load_genomic_breaks("~/FromSango/OSKA2016__AOM-5-5f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, query_bsgenome = BSgenome.Oidioi.OIST.AOM.5.5f, type = "match") |> coalesce_contigs()
AOM10.2f <- load_genomic_breaks("~/FromSango/OSKA2016__AOM-10-2f.gff.gz", target_bsgenome = BSgenome.Oidioi.Aniseed.OSKA2016, type = "match") |> coalesce_contigs()
genome(AOM10.2f$query) <- "AOM10.2f"
```

# Outgroups

Comparisons with more distant genomes that have a chromosomal assembly allow to
verify that we attribute scaffolds to the correct linkage group.

```{r load_outgroup_alignments}
OKI  <- load_genomic_breaks("~/FromSango/OSKA2016__OKI2018_I69_1.0.gff.gz", t = BSgenome.Oidioi.Aniseed.OSKA2016, q = BSgenome.Oidioi.OIST.OKI2018.I69, type = "match") |> coalesce_contigs()
BAR  <- load_genomic_breaks("~/FromSango/OSKA2016__Bar2_p4.gff.gz",         t = BSgenome.Oidioi.Aniseed.OSKA2016, q = BSgenome.Oidioi.OIST.Bar2.p4,     type = "match") |> coalesce_contigs()
```
...
Converting alignments into kind of chains.
See `?longestMatchesInTarget` and `?strandNames` for details.
...
O97f.cc <- strandNames(O97f.c, query = TRUE)
names(O97f.cc) <- paste("O97f", names(O97f.cc), sep = "_")
O97f.all <-  longestMatchesInTarget(O97f, min.width = 0, min.matches = 1) |> strandNames(q=T)
```
...
O35f.cc <- strandNames(O35f.c, query = TRUE)
names(O35f.cc) <- paste("O35f", names(O35f.cc), sep = "_")
O35f.all  <- longestMatchesInTarget(O35f, min.width = 0, min.matches = 0) |> strandNames(q=T)
# 
# O33f.pc <- O33f
...
```{r}
OSK42_2f.c  <- longestMatchesInTarget(OSK42_2f)
OSK42_2f.cc <- strandNames(OSK42_2f.c, query = TRUE)
names(OSK42_2f.cc) <- paste("OSK42_2f", names(OSK42_2f.cc), sep = "_")
OSK42_2f.all  <- longestMatchesInTarget(OSK42_2f, min.width = 0, min.matches = 1) |> strandNames(q=T)
```

## AOM5

```{r}
AOM5.5f.c  <- longestMatchesInTarget(AOM5.5f)
AOM5.5f.cc <- strandNames(AOM5.5f.c, query = TRUE)
names(AOM5.5f.cc) <- paste("AOM5.5f", names(AOM5.5f.cc), sep = "_")
AOM5.5f.all  <- longestMatchesInTarget(AOM5.5f, min.width = 0, min.matches = 1) |> strandNames(q=T)
# 
# 
# shortNamesList(AOM5.5f, 20, min = 1e4)
# 
# 
# # Contains telomere ?
# superScafByName(AOM5.5f,"contig_178", 1e4) |> shortNames()
# #[1] "S25+" "S11-" "S29-"
```

# Function to assemble

Takes a `GRanges` object and returns a `GRanges` object.  BSgenome information
is erased.

The `scaffold` object is a `GRanges` object that describes in which order
and orientation the selected `seqlevels` have to be merged in a new one.

```{r def_assemble}
scaffold <- function(seqinfo, newname, orientedSeqNames) {
  if (!class(seqinfo) %in% "Seqinfo")
    seqinfo <- seqinfo(seqinfo)
  listofnames   <- substr(orientedSeqNames, 1                      , nchar(orientedSeqNames) -1)
  orientations  <- substr(orientedSeqNames, nchar(orientedSeqNames), nchar(orientedSeqNames)   )
  contigsToMerge_start  <- c(1, cumsum(seqlengths(seqinfo)[listofnames]))    |> unname() |> head(-1)
  contigsToMerge_end    <-     (cumsum(seqlengths(seqinfo)[listofnames]) -1) |> unname()
  scaffold         <- GRanges(newname, IRanges(contigsToMerge_start, contigsToMerge_end))
  names(scaffold)  <- listofnames
  strand(scaffold) <- orientations
  scaffold
}

assemble <- function(gr, newname, orientedSeqNames, newGenomeName = NA) {
  if(length(newname) != 1) stop ("Invalid scaffold with more than one seqnames")
  scaffold <- scaffold(gr, newname, orientedSeqNames)
  
  flipScaffold <- function(gr, scaffold) {
    gr[seqnames(gr) == scaffold] <- reverse(gr[seqnames(gr) == scaffold])
    gr
  }
  
  toFlip <- names(scaffold[strand(scaffold) == '-'])
  
  for (seqname in toFlip) {
    gr <- flipScaffold(gr, seqname)
  }
  gr <- mergeSeqLevels(gr, names(scaffold), newname)
  genome(gr) <- newGenomeName
  gr
}
```

# Superscaffolding chromosome arms and sex-specific regions
...
Pairwise alignments to assemblies of the O9, O3, OSK42 and AOM-5 support
...
Thus, the XSR superscaffold above appears to be the best we can produce.


```{r}
XSR.old

XSR <- c("S10+", "S9+", "S32+", "S84-", "S59+", "S8-", "S1+", "S66-")
```

### O9-7f

The O9 individual is form the same laboratory strain as OSKA2016, and its `7f`
assembly was basecalled with the _high accuracy_ model of Guppy.

In O9-7f, the XSR contig is `contig_44` followed by `contig_48`.

`contig_48` continues into S2, which confirms the connection between the XSR and
the long arm of the PAR.

```{r XSR_O97}
O97f.cc |> flipStrandNames() |> grepByContigName("S9+")
O97f |> plyranges::filter(seqnames(query) == "contig_48")|> coalesce_contigs(minwidth = 2000) |> swap() |> sort(i=T) |> swap() |> as.data.frame()
O97f |> plotApairOfChrs(chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S9", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S32", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S84", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S59", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S8", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S1", chrQ = "contig_48")
O97f |> plotApairOfChrs(chrT= "S2", chrQ = "contig_48")

O97f |> plotApairOfChrs(chrT = "S10", chrQ = "contig_44")

O97f |> plyranges::filter(seqnames %in% c("S10", "S9", "S32", "S84", "S59", "S8", "S1", "S2")) |> makeOxfordPlots()
```

### O35f

The O3 individual is form the same laboratory strain as OSKA2016, and its `5f`
assembly was basecalled with the _high accuracy_ model of Guppy.

The XSR is not well assembled in O35f.

```{r XSR_O35f}
O35f.cc |> grepByContigName("S9+")
O35f |> plyranges::filter(seqnames(query) == "contig_45")|> coalesce_contigs(minwidth = 2000) |> swap() |> sort(i=T) |> swap() |> as.data.frame()
O35f |> plotApairOfChrs(chrQ = "contig_45")
O35f |> plotApairOfChrs(chrT= "S9", chrQ = "contig_45")
O35f |> plotApairOfChrs(chrT= "S32", chrQ = "contig_45")
O35f |> plotApairOfChrs(chrT= "S84", chrQ = "contig_45")
O35f |> plotApairOfChrs(chrT= "S59", chrQ = "contig_45")

O35f |> plotApairOfChrs(chrT= "S8", chrQ = "contig_45")
O35f |> plotApairOfChrs(chrT= "S1", chrQ = "contig_48")
O35f |> plotApairOfChrs(chrT= "S2", chrQ = "contig_48")

O35f |> plyranges::filter(seqnames %in% c("S10", "S9", "S32", "S84", "S59", "S8", "S1", "S2")) |> makeOxfordPlots()
assemble(O35f, "XSR", XSR) |> makeOxfordPlots()
```

### OSK42-2f

The OSK-42 individual was sampled in the same region as the Osaka lab strain.
As it is a wild animal, its diploid regions are much harder to assemble.

`contig_33` Fully supports the XSR assembly.

A re-assembly using the _superior_ basecall model might increase support for
the long-arm assembly of the other chromosomes.

```{r OSK_42}
OSK42_2f.cc  |> grepByContigName(name = "S10+")
assemble(OSK42_2f, "XSR", XSR) |> plotApairOfChrs()
assemble(OSK42_2f, "XSR", XSR) |> makeOxfordPlots()
```

### AOM5

```{r XSR_AOM5}
AOM5.5f.cc |> grepByContigName(name = "S10+")
O35f.cc |> grepByContigName("S9+")
...
```{r XSR_check}
sort(XSR)
longestMatchesInTarget(OKI, min.width = 1000, min.matches = 1)["XSR"] |> strandNames(q=T) |> unlist() |> sort()
longestMatchesInTarget(BAR, min.width = 1000, min.matches = 1)["XSR"] |> strandNames(q=T) |> unlist() |> sort()
```

### S66

```{r XSR_S66}
O35f |> subset(seqnames == "S66" & width > 100)|> as.data.frame()
O35f |> subset(seqnames(query) == "scaffold_16" & width > 1000) |> sortQ() |> as.data.frame()   |> tail()
```
...
OSK42_2f.all                      |> grepByContigName(name = "S10+")
AOM5.5f.all  |> flipStrandNames() |> grepByContigName(name = "S10-")
O35f.all |> grepByContigName("S9+")
O35f.all |> flipStrandNames() |> grepByContigName("S10+")
O35f.all |> grepByContigName("S1+")
wannaAdd <- c("S366", "S477", "S510")
O97f |> plyranges::filter(seqnames %in% wannaAdd)
seqlengths(BSgenome.Oidioi.Aniseed.OSKA2016)[wannaAdd]
```

### Summary:

```
XSR.old     "S10+" "S9+"  "S32+" "S84-" "S59+" "S8-"  "S1+"
OSK42_2f.cc "S10+" "S9+"  "S32+" "S84-" "S59+" "S8+"  "S1+" 
AOM5.5f.cc  "S10+" "S9+"  "S32+" "S84-" "S59+" "S8-"  "S1+" 
...
OSK42_2f.all                  "S10+"  "S246+" "S9+"           "S32+"  "S195+" "S84-"         "S59+"  "S494+" "S8+"           "S477+"         "S1+"   "S462-" "S510+" "S564-" "S391+"
AOM5.5f.all  "S307-" "S273-" "S10+"  "S176-" "S9+"           "S32+"          "S84-"         "S59+"          "S8-"   "S366-" "S477+" "S478-" "S1+"   "S517-" "S510+" "S491+" "S66-"  "S481+" "S154+"
...
AOM5.5f.cc |> grepByContigName("S7")
...
AOM5.5f.cc |> grepByContigName("S82")
...
Luckily, O35f's scaffold 48 is the whole long arm of Chr2.

```{r chr2_long_O3}
flipStrandNames(O35f.cc)$O35f_scaffold_48
O35f |> subset(seqnames(query) == "scaffold_48" & width > 100000) |> sortQ() |> as.data.frame()

# And S4's tail aligns to a contig ending with a telomere !
O35f |> subset(seqnames == "S4" & width > 100000)|> as.data.frame()

# Dead end on the other side.
O35f |> subset(seqnames == "S14" & width > 100000)|> as.data.frame()

# But it allows to place S60, although with low confidence
O35f |> subset(seqnames == "S60" & width > 100)|> as.data.frame() |> head()
O35f |> subset(seqnames(query) == "scaffold_48" & width > 1000) |> sortQ() |> as.data.frame()   |> tail()


assemble(O35f, "XSR", XSR) |>
  assemble("chr2", c("S4+", "S19-", "S24+", "S75+", "S27-", "S40-", "S72+", "S46-", "S35-", "S22+", "S14+")) |>
  plyranges::filter(score > 1e4) |> swap()|> makeOxfordPlots()
```

#### AOM5.5f

```{r chr2_long_AOM5}
AOM5.5f.cc |> grepByContigName("S4+")
AOM5.5f.cc |> grepByContigName("S17")
assemble(AOM5.5f, "XSR", XSR) |>
  assemble("chr2", c("S4+", "S19-", "S24+", "S75+", "S27-", "S40-", "S72+", "S46-", "S35-", "S22+", "S14+")) |>
  plyranges::filter(score > 1e4) |> swap()|> makeOxfordPlots()
```
...
```{r chr2_long}
OSK42_2f.cc |> grepByContigName("S82")
OSK42_2f.cc |> grepByContigName("S17")
```
...
```{r}
AOM5.5f.cc |> grepByContigName("S11")
OSK42_2f.cc |> grepByContigName("S33")
...
AOM5.5f.cc |> grepByContigName("S25")
OSK42_2f.cc |> grepByContigName("S25")
...
AOM5.5f.cc |> grepByContigName("S13")
OSK42_2f.cc |> grepByContigName("S13")
...
AOM5.5f.cc |> grepByContigName("S18")
OSK42_2f.cc |> grepByContigName("S23")
...
OSK42_2f.cc |>  flipStrandNames |> grepByContigName("S26")
AOM5.5f.cc |> flipStrandNames |> grepByContigName("S26")
...
AOM5.5f.cc                                             "S48-" "S55+"                                                                       "S71-" "S47+" "S26+"          "S64-"
```
...
OSK42_2f |> subset(seqnames == "S51" & width > 1000)
OSK42_2f |> subset(seqnames(query) == "contig_306" & width > 2000) |> sortQ |> as.data.frame()
```
...
OSK42_2f |> subset(seqnames == "S67" & width > 1000)
OSK42_2f |> subset(seqnames(query) == "contig_5" & width > 2000) |> sortQ |> as.data.frame()
```
...
OSK42_2f |> subset(seqnames == "S99" & width > 100)
OSK42_2f |> subset(seqnames(query) == "contig_98" & width > 1000) |> sortQ |> as.data.frame()
OSK42_2f |> subset(seqnames(query) == "contig_3" & width > 1000) |> sortQ |> as.data.frame()
```
...
assembleAll <- \(gb) gb |>
  assemble("XSR", XSR)   |> assemble("YSR", YSR) |> assemble("PAR", PAR) |>
  assemble("chr1", chr1) |> assemble("chr2", chr2)
```

## O9

```{r summary_O9}
O97f |> assembleAll() |> plyranges::filter(score > 1e4) |> makeOxfordPlots()
```

## O3

```{r summary_O9}
O35f |> assembleAll() |> plyranges::filter(score > 1e4) |> makeOxfordPlots()
```

# Roadmap towards OSKA2016v2

 - Orient `XSR` so that the telomere is at the 3' end, or
 - Merge the `PAR` and the (re-oriented) `XSR` to form `chrX.`
