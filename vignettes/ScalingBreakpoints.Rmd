...
bp_vs_dist <- function(query, target, wgo, dist_type="mya", species){
  ...
  breakpoint_df <- data.frame(species=species, query=query, target=target, pair=paste0(query, "_", target), dist_type=dist_type, dist=relevant_dist, breakpoints=length(breakpoints), breakpoints_per_mbp=breakpoints_per_mbp, breakpoints_per_mbp_scaled=breakpoints_per_mbp_per_dist)
...
mya_df$species <- factor(mya_df$species, levels=sort(mya_df$species))
...
sub_df$species <- factor(sub_df$species, levels=sort(sub_df$species))
