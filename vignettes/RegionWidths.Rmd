...
```{r saving_width_statistics_for_later, dependson="width_after_processing_steps"}
library(dplyr) |> suppressPackageStartupMessages()

norm_CI <- function(vector, conf.level){
...
wilcox_CI <- function(vector, conf.level){
  conf_test <- wilcox.test(vector, conf.level = conf.level)
  conf_str <- paste(round(conf_test$conf.int,2), collapse=',')
  conf_str
}

stats_to_compute <- list(n         = ~n(),
                         norm_CI   = ~norm_CI(., conf.level=0.95),
                         wilcox_CI = ~norm_CI(., conf.level=0.95),
                         min       = min,
                         q25       = ~quantile(., 0.25),
                         median    = median,
                         q75       = ~quantile(., 0.75),
...
p_unalmap2 <- gg_freq_poly(widths$bri |> dplyr::filter(dist == "Oki – North")) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
