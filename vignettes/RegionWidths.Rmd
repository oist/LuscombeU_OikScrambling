...
tra <- sapply(coa, flagTranslocations)
tra_only <- sapply(tra, filterTranslocations)
sapply(tra_only, length)
widths$tra <- makeWidthsDF(tra_only) |> tibble::as_tibble()
widths$tra_op <- makeWidthsDF(sapply(tra_only, swap)) |> tibble::as_tibble()
...
gg_freq_poly(widths$tra_op) +
  ggtitle("Distribution of widths of translocated regions (opposite side)")

gg_freq_poly(widths$tra |> subset(chr == "chr1")) +
  facet_wrap(~name, scales = "free_y", ncol=5) +
  ggtitle("Distribution of widths of translocated regions on chr1 after collapsing, split by genome pair") +
  geom_vline(xintercept = 500)
```
...
tra.w2s     <- do.call(rbind, lapply(names(tra_only), \(name) GBreaks2widthTibble(tra_only[[name]], pair = name)))
...
ggPlotW2W(tra.w2s |> dplyr::filter(!is.na(Arm))) + facet_wrap(~Arm) +
...
widths$invs <- makeWidthsDF(invs) |> tibble::as_tibble()
