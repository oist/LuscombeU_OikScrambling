...
```{r accessory_functions_and_values}
...
distance group resemble each other.
...
widths_all_tibble <- rbind(widths$gbs |> dplyr::mutate(what = "aligned"),
                       ...
                       widths$unalMap |> dplyr::mutate(what = "mapped unaligned"),
...
```{r width_after_processing_steps_by_class_and_genus, dependson="width_after_processing_steps", fig.width=12}
...
## Details 
<details>
<summary> Click here to view the full table with many summary statistics for width distributions.</summary>
```{r saving_width_statistics_for_later, dependson="pre_compute_widths_object", fig.width=12, fig.height=8}
widths_stats <- widths_all_tibble %>%
    group_by(genus, class, dist, distS, what, name, nonCoa, chr, Arm) %>%
    summarise(across(width, list(n=~n(), min=min, q25 = ~quantile(., 0.25),  median=median, q75 = ~quantile(., 0.75), max=max, mean=mean, sd=sd, var=var)))
(widths_stats)
```
</details>
...
```{r width_before_coalescing, dependson="pre_compute_widths_object", fig.width=10}
...
```{r width_before_coalescing2, dependson="pre_compute_widths_object", fig.height=9, fig.width=6}
gg_freq_poly(widths$gbs) +
...
Note that for this reason, it would be unwise to filter by width on some
operations such as coalescing.

```{r widths_gbs_by_arm, dependson="pre_compute_widths_object"}
gg_freq_poly(widths$gbs) +
  aes(col=Arm) +
...
Width distribution per arm follows similar pattern on chr2 (first graph).  Pairs
at a similar evolutionary distance show similar profile, with the exception
of Osa_Aom, where the number of alignments on short arms is higher.
Nevertheless, the trend of widths being longer on long arms stay true, so
we can average them in the plot below.

```{r widths_gbs2_by_arm, dependson="pre_compute_widths_object"}
...
Interestingly, only in Oikopleura do the width distributions from pairs of
different distance peak at the same size.  Could that be because the genome is
very compact?

```{r widths_unaligned2, dependson="pre_compute_widths_object"}
gg_freq_poly(widths$unal |> subset(chr %in% c("chr1", "chr2", "PAR"))) +
  aes(col=name) +
  facet_wrap(~dist, scales = "free_y", ncol=1) +
  ggtitle("Distribution of unaligned region widths") +
...
After coalescing, some width distribution become clearly bimodal.
The distributions are hard to compare between genera and between evolutionary
distance groups, and even within group for the "same population" _Oikopleura_.

The plots below display the data in details for the reader to make their
opinion about the above statement.

### Whole data

#### Width distributions.

```{r width_after_coalescing, dependson="pre_compute_widths_object", fig.height=8, fig.width=12}
...
```{r width_after_coalescing2, dependson="pre_compute_widths_object", fig.height=12, fig.width=8}
gg_freq_poly(widths$coa) +
  aes(width, after_stat(density), col=class) +
  facet_wrap(~genus, scales = "free_y", ncol = 1) +
  ggtitle("Distribution of mapped region widths")
...
```{r width_after_coalescing_ecdf, dependson="pre_compute_widths_object", fig.height=8, fig.width=12}
...
```{r width_after_coalescing_cumsum, dependson="pre_compute_widths_object", fig.height=8, fig.width=12}
widths$coa |>
...
When faceted by chromosome and arm type, bimodality is also observed.  There is
a small peak under 500 bp, found on all arms except YSR.  The second peak shows
two different distributions for long and short arms.

```{r width_after_coalescing3, dependson="pre_compute_widths_object"}
gg_freq_poly(widths$coa) +
  aes(col=Arm) +
  facet_wrap(~chr, scales = "free_y") +
...
The mapped regions that are found identical in the aligned set are those that
were not coalesced... Are they the small peak ?

```{r non_coalesced_mapped_regions}
...
```{r mapped_region_widths_Oki_vs_North}
...
We know from `vignette("RepeatRegions", package = "OikScrambling")` that these
short mapped regions are not repeats.

### Compound pictures

Let's put into parallel "aligned", "unaligned" and "mapped"
...
```{r compound2}
p_ali2 <- gg_freq_poly(widths$gbs |> dplyr::filter(dist == "Oki – North", nonCoa)) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Isolated alignments")

p_una2 <- gg_freq_poly(widths$unmap |> dplyr::filter(dist == "Oki – North")) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Not aligned, not mapped")

p_syn2 <- gg_freq_poly(widths$coa |> dplyr::filter(dist == "Oki – North", !nonCoa)) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Synteny block")

p_map2 <- gg_freq_poly(widths$gbs |> dplyr::filter(dist == "Oki – North", !nonCoa)) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Synteny alignment")

p_unalmap2 <- gg_freq_poly(widths$unalMap |> dplyr::filter(dist == "Oki – North")) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Unaligned but syntenic")

# Patchwork
(p_ali2 | p_una2 | p_syn2 | p_map2 | p_unalmap2) + plot_layout(ncol = 5, guides = 'collect') + theme_bw()
```
...
```{r width_of_translocated_regions_rem, dependson="pre_compute_widths_object"}
...
```{r inversion_width_aligned, dependson="pre_compute_widths_object"}
...
```{r inversion_width_mapped, dependson="pre_compute_widths_object"}
...
```{r inversion_width_doublemapped, dependson="pre_compute_widths_object"}
