...
p_unmap2 <- gg_freq_poly(widths$unmap |> dplyr::filter(dist == "Oki – North")) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Unmapped")
...
p_bridge <- gg_freq_poly(widths$bri |> dplyr::filter(dist == "Oki – North")) +
  aes(width, after_stat(density), col=name) + theme_bw() +
  scale_x_log10(limits = c(1,5e5)) +
  ggtitle("Bridge")

# Patchwork
(p_ali2 | p_unmap2 | p_syn2 | p_bridge | p_map2 ) + plot_layout(ncol = 5, guides = 'collect') + theme_bw()
```
