...
compare_exact_operons_by_HOGs <- function(op_sp1, op_sp2, op_sp3, sp_names=c("Oki", "Osa", "Bar")) {
  ...
  operon_sets <- list(hog_characterlist_to_hog_id(op_sp1, cl_name = "HOGs", sp=sp_names[[1]]),
...
compare_exact_operons_by_OGs <- function(op_sp1, op_sp2, op_sp3, sp_names=c("Oki", "Osa", "Bar")) {
  ...
  operon_sets <- list(hog_characterlist_to_hog_id(op_sp1, cl_name = "OGs", sp=sp_names[[1]]),
...
hog_characterlist_to_hog_id(subsetByOverlaps(operons_hogs$Oki, GRanges("PAR:9771465-9794560")), cl_name = "HOGs", sp = "Oki")
```
...
stringdist_of_operon_against_all(hog_characterlist_to_hog_id(operons_hogs_utf$Oki$utf_words[1]), hog_characterlist_to_hog_id(operons_hogs_utf$Osa$utf_words)) |> head()

stringdist_all_by_all <- function(opl_sp1, opl_sp2, nthread=1, method="lv", mc.cores=1) {
...
lapply(stringdist_all_by_all(hog_characterlist_to_hog_id(operons_hogs_utf$Oki$utf_words[1:3]), hog_characterlist_to_hog_id(operons_hogs_utf$Osa$utf_words)), head)

stringdist_all_by_all_test <- function(stringdists, max_distance=0){
  sds <- lapply(1:length(stringdists), function(s) {
    if(min(s) <= max_distance){
      return(s)
    }
  })
  unlist(sds)
}
stringdist_all_by_all(hog_characterlist_to_hog_id(operons_hogs_utf$Oki$utf_words[1:1000]), hog_characterlist_to_hog_id(operons_hogs_utf$Osa$utf_words), mc.cores = 8) |> stringdist_all_by_all_test()

# Use UTF-8-encoded operons as phrases, then compare inexact matches with string distances
compare_inexact_operons_by_stringdist <- function(op_sp1, op_sp2, op_sp3, sp_names=c("Oki", "Osa", "Bar"), method="lv", max_distance=0, nthread = 1, min_size=2) {
  # Don't let the function be run on an un-annotated operons() GRanges
  ...
  result$sd_sp1_sp2 <- sd_sp1_sp2
  result$sd_sp2_sp3 <- sd_sp2_sp3
  result$sd_sp1_sp3 <- sd_sp1_sp3
  result$sd_sp1_sp2_sp3 <- intersect(intersect(sd_sp1_sp2, sd_sp2_sp3), sd_sp1_sp3)
  result
}

inexact_operons <- compare_inexact_operons_by_stringdist(operons_hogs_utf$Oki, operons_hogs_utf$Osa, operons_hogs_utf$Bar, sp_names=c("Oki", "Osa", "Bar"), max_distance = 1, min_size=3, nthread=8)
names(inexact_operons) <- c("Oki", "Osa", "Bar", "Oki&Osa", "Osa&Bar", "Oki&Bar", "Oki&Osa&Bar")
