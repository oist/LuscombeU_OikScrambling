---
title: "Operon conservation analysis"
author: 
 - "Michael Mansfield"
 - "Charles Plessy"
...
vignette: >
  ...
  %\VignetteIndexEntry{Operon conservation analysis}
...
The following function generates sets of collinear genes separated by at most
`window` base pairs on either side.  See the `vignette("OperonDetection",
package = "OikScrambling")` for details.
...
#eulerr::euler(c("Oki"=1178, "Osa"=1025, "Bar"=928, "Oki&Osa"=107, "Oki&Bar"=119, "Osa&Bar"=168, "Oki&Osa&Bar"=38)) |> plot(quantities=TRUE, main="Number of operons >= 3 genes, with >=1 gene different\ncalcOperons window=500bp", fills=sp_fills, col=sp_cols)

eulerr::euler(c(
  "Oki"=inexact_operons_tibble_ggvenn |> filter(Oki==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Osa"=inexact_operons_tibble_ggvenn |> filter(Osa==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Bar"=inexact_operons_tibble_ggvenn |> filter(Bar==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Osa"=inexact_operons_tibble_ggvenn |> filter(Oki_Osa==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Bar"=inexact_operons_tibble_ggvenn |> filter(Oki_Bar==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Osa&Bar"=inexact_operons_tibble_ggvenn |> filter(Osa_Bar==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Osa&Bar"=inexact_operons_tibble_ggvenn |> filter(Oki_Osa_Bar==TRUE)  |> nrow()
  )
) |> plot(main="Number of operons >= 3 genes (HOGs), with >=1 gene different\ncalcOperons window=500bp", fills=sp_fills, col=sp_cols, quantities=list(type = c('counts',"percent")))

```


```{r inexact_match_operons_2}
og_ids_to_utf8 <- function(og, sp1, sp2, sp3, sp_names=c("Oki", "Osa", "Bar")) {
  sp1$sp_id <- sp2$sp_id <- sp3$sp_id <- NA
  # First species' words
  sp1$sp_id[which(!is.na(sp1$OG))] <- sp1$OG[which(!is.na(sp1$OG))]
  sp1$sp_id[which(is.na(sp1$OG))] <- paste0(sp_names[[1]], "__", sp1$gene_id[which(is.na(sp1$OG))])
  # Second species' words
  sp2$sp_id[which(!is.na(sp2$OG))] <- sp2$OG[which(!is.na(sp2$OG))]
  sp2$sp_id[which(is.na(sp2$OG))] <- paste0(sp_names[[2]], "__", sp2$gene_id[which(is.na(sp2$OG))])
  # Third species' words
  sp3$sp_id[which(!is.na(sp3$OG))] <- sp3$OG[which(!is.na(sp3$OG))]
  sp3$sp_id[which(is.na(sp3$OG))] <- paste0(sp_names[[3]], "__", sp3$gene_id[which(is.na(sp3$OG))])
...
genes_ogs_utf <- og_ids_to_utf8(pho, genes_hogs$Oki, genes_hogs$Osa, genes_hogs$Bar)

# Annotate operons GR with UTF-8 words
annotate_operons_with_ogs_utf <- function(op_gr, og_utf_gr) {
  if(is.null(names(op_gr))){
    names(op_gr) <- paste0("o", 1:length(op_gr))
  }
  op_gr_annot <- op_gr$gene_id |> as.data.frame()
  op_gr_annot$HOG <- og_utf_gr$HOG[match(op_gr_annot$value, og_utf_gr$gene_id)]
  op_gr_annot$OG <- og_utf_gr$OG[match(op_gr_annot$value, og_utf_gr$gene_id)]
  op_gr_annot$utf_word <- og_utf_gr$utf_word[match(op_gr_annot$value, og_utf_gr$gene_id)]
...
operons_ogs_utf <- SimpleList()
operons_ogs_utf$Oki <- annotate_operons_with_ogs_utf(operons$Oki, genes_ogs_utf$Oki)
operons_ogs_utf$Osa <- annotate_operons_with_ogs_utf(operons$Osa, genes_ogs_utf$Osa)
operons_ogs_utf$Bar <- annotate_operons_with_ogs_utf(operons$Bar, genes_ogs_utf$Bar)

stringdist_of_operon_against_all(hog_characterlist_to_hog_id(operons_ogs_utf$Oki, cl_name = "utf_words")[1], hog_characterlist_to_hog_id(operons_ogs_utf$Osa, cl_name="utf_words")) |> head()
...
lapply(stringdist_of_allop_vs_allop(hog_characterlist_to_hog_id(operons_ogs_utf$Oki, cl_name = "utf_words")[1:3], hog_characterlist_to_hog_id(operons_ogs_utf$Osa, cl_name="utf_words")), head)
#tmpsd <- stringdist_of_allop_vs_allop(hog_characterlist_to_hog_id(operons_hogs_utf$Oki, cl_name = "utf_words"), hog_characterlist_to_hog_id(operons_hogs_utf$Osa, cl_name="utf_words"), mc.cores = 4)

stringdist_of_allop_vs_allop(hog_characterlist_to_hog_id(operons_ogs_utf$Oki, cl_name = "utf_words")[1:10], hog_characterlist_to_hog_id(operons_ogs_utf$Osa, cl_name="utf_words"), mc.cores = 4) |> stringdist_all_by_all_test()

inexact_operons_ogs <- compare_inexact_operons_by_stringdist(operons_ogs_utf$Oki, operons_ogs_utf$Osa, operons_ogs_utf$Bar, sp_names=c("Oki", "Osa", "Bar"), max_distance = 1, min_size=3, mc.cores=4)

inexact_operons_tibble_ggvenn2 <- data.frame(operons=unique(unlist(inexact_operons_ogs))) |> dplyr::as_tibble() 
inexact_operons_tibble_ggvenn2$Oki <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$Oki
inexact_operons_tibble_ggvenn2$Osa <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$Osa
inexact_operons_tibble_ggvenn2$Bar <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$Bar
inexact_operons_tibble_ggvenn2$Oki_Osa <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$inexact_sp1_sp2
inexact_operons_tibble_ggvenn2$Oki_Bar <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$inexact_sp1_sp3
inexact_operons_tibble_ggvenn2$Osa_Bar <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$inexact_sp2_sp3
inexact_operons_tibble_ggvenn2$Oki_Osa_Bar <- inexact_operons_tibble_ggvenn2$operons %in% inexact_operons_ogs$inexact_sp1_sp2_sp3


apply(inexact_operons_tibble_ggvenn2[,2:8], 2, table)
apply(apply(inexact_operons_tibble_ggvenn2[,2:8], 2, table), 2, sum)

#eulerr::euler(c("Oki"=1178, "Osa"=1025, "Bar"=928, "Oki&Osa"=118, "Oki&Bar"=130, "Osa&Bar"=181, "Oki&Osa&Bar"=39)) |> plot(quantities=TRUE, main="Number of operons >= 3 genes (OGs), with >=1 gene different\ncalcOperons window=500bp", fills=sp_fills, col=sp_cols)

eulerr::euler(c(
  "Oki"=inexact_operons_tibble_ggvenn2 |> filter(Oki==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Osa"=inexact_operons_tibble_ggvenn2 |> filter(Osa==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Bar"=inexact_operons_tibble_ggvenn2 |> filter(Bar==TRUE, Oki_Osa==FALSE, Oki_Osa==FALSE, Osa_Bar==FALSE, Oki_Bar==FALSE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Osa"=inexact_operons_tibble_ggvenn2 |> filter(Oki_Osa==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Bar"=inexact_operons_tibble_ggvenn2 |> filter(Oki_Bar==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Osa&Bar"=inexact_operons_tibble_ggvenn2 |> filter(Osa_Bar==TRUE, Oki_Osa_Bar==FALSE)  |> nrow(),
  "Oki&Osa&Bar"=inexact_operons_tibble_ggvenn2 |> filter(Oki_Osa_Bar==TRUE)  |> nrow()
  )
) |> plot(main="Number of operons >= 3 genes (OGs), with >=1 gene different\ncalcOperons window=500bp", fills=sp_fills, col=sp_cols, quantities=list(type = c('counts',"percent")))

```

# Operon swapping
Are genes in operons more or less likely to end up in another operon?

```{r operon_swapping_1}
# Number of genes 
n_genes <- length(genes_hogs$Oki)
# Number of operons
n_operons <- length(operons_hogs$Oki)
# Number of genes in operons
n_genes_in_operons <- length(unlist(operons_hogs$Oki$gene_id))
# Number of genes in HOGs
n_genes_in_hogs <- length(na.omit(genes_hogs$Oki$HOG))
# Number of genes in operons in HOGs
n_genes_in_operons_in_hogs <- length(na.omit(unlist(operons_hogs$Oki$HOGs)))
# Fraction of genes with HOGs
n_genes_in_hogs/n_genes
# Fraction of genes in operons with HOGs
n_genes_in_operons_in_hogs/n_genes_in_operons
# This is not substantially different. The number of genes assigned to HOGs is not very different in vs. out of operons.

# Subset to genes with shared HOGs
oki_hogs <- genes_hogs_utf$Oki[genes_hogs_utf$Oki$HOG %in% genes_hogs_utf$Osa$HOG]
oki_hogs <- oki_hogs[oki_hogs$HOG %in% genes_hogs_utf$Bar$HOG]
in_operon_oki  <- oki_hogs[ oki_hogs$gene_id %in%  unlist(operons_hogs_utf$Oki$gene_id)]
out_operon_oki <- oki_hogs[!oki_hogs$gene_id %in%  unlist(operons_hogs_utf$Oki$gene_id)]

osa_hogs <- genes_hogs_utf$Osa[genes_hogs_utf$Osa$HOG %in% genes_hogs_utf$Oki$HOG]
osa_hogs <- osa_hogs[osa_hogs$HOG %in% genes_hogs_utf$Bar$HOG]
in_operon_osa  <- osa_hogs[ osa_hogs$gene_id %in%  unlist(operons_hogs_utf$Osa$gene_id)]
out_operon_osa <- osa_hogs[!osa_hogs$gene_id %in%  unlist(operons_hogs_utf$Osa$gene_id)]

bar_hogs <- genes_hogs_utf$Bar[genes_hogs_utf$Bar$HOG %in% genes_hogs_utf$Oki$HOG]
bar_hogs <- bar_hogs[bar_hogs$HOG %in% genes_hogs_utf$Oki$HOG]
in_operon_bar  <- bar_hogs[ bar_hogs$gene_id %in%  unlist(operons_hogs_utf$Bar$gene_id)]
out_operon_bar <- bar_hogs[!bar_hogs$gene_id %in%  unlist(operons_hogs_utf$Bar$gene_id)]


# Then evaluate whether genes in operons are more likely to be in operons in another species.

# How many genes are in an operon in one species but not the other species?
chisq_oki_osa <- rbind(
  na.omit(in_operon_oki$utf_word) %in% unlist(in_operon_osa$utf_word) |> table(),
  na.omit(in_operon_oki$utf_word) %in% unlist(out_operon_osa$utf_word) |> table(),
  na.omit(out_operon_oki$utf_word) %in% unlist(in_operon_osa$utf_word) |> table(),
  na.omit(out_operon_oki$utf_word) %in% unlist(out_operon_osa$utf_word) |> table()
) |> as.data.frame()
rownames(chisq_oki_osa) <- c("inOki_inOsa", "inOki_outOsa", "outOki_inOsa", "outOki_outOsa")
chisq_oki_osa
# Plot
p1 <- chisq_oki_osa  |> tibble::rownames_to_column("operon_status") |> tidyr::pivot_longer(cols=2:3) |> dplyr::rename(n_genes=value, value=name) |> ggplot() + aes(y=operon_status, x=n_genes, fill=value) + geom_bar(stat='identity') + ggtitle("Oki-Osa: Homologous genes by operon membership")
p1
# Stats
chisq_oki_osa_test <- chisq.test(chisq_oki_osa)
lapply(chisq_oki_osa_test, \(x) x)
chisq_oki_osa_test <- AMR::g.test(chisq_oki_osa)
lapply(chisq_oki_osa_test, \(x) x)


chisq_oki_bar <- rbind(
  na.omit(in_operon_oki$utf_word) %in% unlist(in_operon_bar$utf_word) |> table(),
  na.omit(in_operon_oki$utf_word) %in% unlist(out_operon_bar$utf_word) |> table(),
  na.omit(out_operon_oki$utf_word) %in% unlist(in_operon_bar$utf_word) |> table(),
  na.omit(out_operon_oki$utf_word) %in% unlist(out_operon_bar$utf_word) |> table()
) |> as.data.frame()
rownames(chisq_oki_bar) <- c("inOki_inBar", "inOki_outBar", "outOki_inBar", "outOki_outBar")
chisq_oki_bar
# Plot
p2 <- chisq_oki_bar  |> tibble::rownames_to_column("operon_status") |> tidyr::pivot_longer(cols=2:3) |> dplyr::rename(n_genes=value, value=name) |> ggplot() + aes(y=operon_status, x=n_genes, fill=value) + geom_bar(stat='identity') + ggtitle("Oki-Bar: Homologous genes by operon membership")
p2
# Stats
chisq_oki_bar_test <- chisq.test(chisq_oki_bar)
lapply(chisq_oki_bar_test, \(x) x)
chisq_oki_osa_test <- AMR::g.test(chisq_oki_bar)
lapply(chisq_oki_osa_test, \(x) x)

chisq_osa_bar <- rbind(
  na.omit(in_operon_osa$utf_word) %in% unlist(in_operon_bar$utf_word) |> table(),
  na.omit(in_operon_osa$utf_word) %in% unlist(out_operon_bar$utf_word) |> table(),
  na.omit(out_operon_osa$utf_word) %in% unlist(in_operon_bar$utf_word) |> table(),
  na.omit(out_operon_osa$utf_word) %in% unlist(out_operon_bar$utf_word) |> table()
) |> as.data.frame()
rownames(chisq_osa_bar) <- c("inOsa_inBar", "inOsa_outBar", "outOsa_inBar", "outOsa_outBar")
chisq_osa_bar
p3 <- chisq_osa_bar  |> tibble::rownames_to_column("operon_status") |> tidyr::pivot_longer(cols=2:3) |> dplyr::rename(n_genes=value, value=name) |> ggplot() + aes(y=operon_status, x=n_genes, fill=value) + geom_bar(stat='identity') + ggtitle("Osa-Bar: Homologous genes by operon membership")
p3
# Stats
chisq_osa_bar_test <- chisq.test(chisq_osa_bar)
lapply(chisq_osa_bar_test, \(x) x)
chisq_osa_bar_test <- AMR::g.test(chisq_osa_bar)
lapply(chisq_osa_bar_test, \(x) x)
